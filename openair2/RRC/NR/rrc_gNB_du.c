...
void rrc_gNB_process_f1_setup_req(f1ap_setup_req_t *req, sctp_assoc_t assoc_id)
{
  ...
  SM_Logs(LOG_INFO, _RRC_, "F1 Setup Request detected from DU. Initiating integrity checks and preparing necessary configurations.");
  ...
  if (req->num_cells_available != 1) {
    ...
    SM_Logs(LOG_ERROR,_RRC_,"gNB DU %ld reported more cells (%d) than can be handled\n", req->gNB_DU_id, req->num_cells_available);
  ...
  if (!rrc_gNB_plmn_matches(rrc, cell_info)) {
    ...
    //       cell_info->plmn.mnc);
    SM_Logs(LOG_ERROR,_RRC_, "Plmn mismatch \n CU : (%03d.%0*d)\n DU : (%03d.%0*d)\n", rrc->configuration.mcc[0], rrc->configuration.mnc_digit_length[0], rrc->configuration.mnc[0], cell_info->plmn.mcc, cell_info->plmn.mnc_digit_length, cell_info->plmn.mnc);
  ...
  RB_FOREACH(it, rrc_du_tree, &rrc->dus) {
    ...
    if (exist_info->nr_cellid == new_info->nr_cellid || exist_info->nr_pci == new_info->nr_pci) {
      ...
      //       new_info->nr_cellid,
      //       new_info->nr_pci);
      SM_Logs(LOG_ERROR,_RRC_,"Conflict detected ! DU %s, configured with cellid %ld, unable to process request with cellid %ld\n", it->setup_req->gNB_DU_name, exist_info->nr_cellid, new_info->nr_cellid);
  ...
  SM_Logs(LOG_INFO, _RRC_, " Central Unit (CU) accepting Distributed Unit (DU). Dispatching F1 Setup Response to initiate further configuration.");

  // LOG_I(RRC, "DU uses RRC version %u.%u.%u\n", req->rrc_ver[0], req->rrc_ver[1], req->rrc_ver[2]);
  SM_Logs(LOG_INFO, _RRC_, "Deploying RRC protocol sequence. Distributed Unit (DU) utilizes the latest RRC version for session management and signaling.");
...
static void update_cell_info(nr_rrc_du_container_t *du, const f1ap_served_cell_info_t *new_ci)
{
  ...
  if (new_mtc != NULL) {
    ...
    SM_Logs(LOG_ERROR,_RRC_,"Failed to decode MeasTimingConfig during cell update\n");
