...
//------------------------------------------------------------------------------
{
  ...
  if (selected_plmn_identity != 0){
    SM_Logs(LOG_ERROR,_NGAP_,"Selected PLMN identity %ld is invalid\n", rrcSetupComplete->selectedPLMN_Identity);
  }
...
static int decodePDUSessionResourceSetup(pdusession_t *session)
{
  ...
  if (dec_rval.code != RC_OK) {
    ...
    SM_Logs(LOG_ERROR,_RRC_,"Failed to decode resourcesetupreq \n");
    return -1;
  }
...
void trigger_bearer_setup(gNB_RRC_INST *rrc, gNB_RRC_UE_t *UE, int n, pdusession_t *sessions, uint64_t ueAggMaxBitRateDownlink)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    SM_Logs(LOG_INFO, _RRC_, "Adding PDU session with ID %d, total number of sessions: %d\n", session->pdusession_id, UE->nb_of_pdusessions);
...
void rrc_gNB_send_NGAP_PDUSESSION_SETUP_RESP(gNB_RRC_INST *rrc, gNB_RRC_UE_t *UE, uint8_t xid)
{
  ...
  if ((pdu_sessions_done > 0 || pdu_sessions_failed)) {
    SM_Logs(LOG_INFO, _RRC_, "NGAP_PDUSESSION_SETUP_RESP: Transmitting the response message\n");
...
//------------------------------------------------------------------------------
{
  ...
  SM_Logs(LOG_INFO, _RRC_, "UE with ID %d received a PDU session setup request\n", UE->rrc_ue_id);


  if (!UE->as_security_active) {
  //  LOG_E(NR_RRC, "UE %d: no security context active for UE, rejecting PDU session setup request\n", UE->rrc_ue_id);
    SM_Logs(LOG_ERROR,_RRC_,"Pdu session setup request rejected, no security context for the UE\n");
...
//------------------------------------------------------------------------------
{
  if (ue_context_pP == NULL) {
    ...
    SM_Logs(LOG_INFO,_RRC_,"Received Ngap UE Context Release Request");
    SM_Logs(LOG_ERROR,_RRC_,"UE context is invalid\n");
