...
//-----------------------------------------------------------------------------
{
  ...
  // SM_Logs(LOG_INFO,_RRC_,"Sending RRC Setup Msg\n");
...
static void rrc_gNB_generate_dedicatedRRCReconfiguration(gNB_RRC_INST *rrc, gNB_RRC_UE_t *ue_p)
{
  ...
  // LOG_UE_DL_EVENT(ue_p, " RRC_Reconfiguration bytes %d, id %d)\n", size, xid);
                                       ...
                                       const f1ap_initial_ul_rrc_message_t *msg)
{
  ...
  if (NR_InitialUE_Identity_PR_randomValue == rrcSetupRequest->ue_Identity.present) {
    ...
    if (rrcSetupRequest->ue_Identity.choice.randomValue.size != 5) { // 39-bit random value
      ...
      SM_Logs(LOG_ERROR,_RRC_,"Invalid initUEid");
      return;
    }
    ...
    if (rrcSetupRequest->ue_Identity.choice.ng_5G_S_TMSI_Part1.size != 5) {
      ...
      SM_Logs(LOG_ERROR,_RRC_,"Invalid TMSI_PART1");
      return;
    }
...
void rrc_forward_ue_nas_message(gNB_RRC_INST *rrc, gNB_RRC_UE_t *UE)
{
 ...
 // LOG_UE_DL_EVENT(UE, "Send DL Information Transfer [%d bytes]\n", UE->nas_pdu.length);
  SM_Logs(LOG_INFO,_RRC_,"Sending DL_INFO_TRANSFER");
...
static void handle_ueCapabilityInformation(gNB_RRC_INST *rrc, gNB_RRC_UE_t *UE, const NR_UECapabilityInformation_t *ue_cap_info)
{
  ...
  if (a != RRC_UECAPABILITY_ENQUIRY) {
    ...
    SM_Logs(LOG_ERROR,_RRC_,"Received Unexpected UE Capability Information");
    return;
  }
    ...
    for (int i = 0; i < ue_CapabilityRAT_ContainerList->list.count; i++) {
      ...
      if (ue_cap_container->rat_Type == NR_RAT_Type_nr) {
        ...
        if ((dec_rval.code != RC_OK) && (dec_rval.consumed == 0)) {
          ...
          SM_Logs(LOG_ERROR,_RRC_,"Failed to decode UE capabilities");
          ASN_STRUCT_FREE(asn_DEF_NR_UE_NR_Capability, UE->UE_Capability_nr);
          UE->UE_Capability_nr = 0;
        }
...
static void handle_rrcSetupComplete(gNB_RRC_INST *rrc, gNB_RRC_UE_t *UE, const NR_RRCSetupComplete_t *setup_complete)
{
  ...
  if (setup_complete->criticalExtensions.present != NR_RRCSetupComplete__criticalExtensions_PR_rrcSetupComplete) {
    ...
    SM_Logs(LOG_ERROR,_RRC_,"Received invalid or malformed RRCSetupComplete");
    return;
  }
  ...
  if (setup_complete_ies->ng_5G_S_TMSI_Value != NULL) {
    ...
    if (setup_complete_ies->ng_5G_S_TMSI_Value->present == NR_RRCSetupComplete_IEs__ng_5G_S_TMSI_Value_PR_ng_5G_S_TMSI_Part2) {
      ...
      if (part2->size != 2) {
        SM_Logs(LOG_ERROR,_RRC_,"Invalid TMSI_PART2");
      ...
      if (bs_stmsi->size != 6) {
        ...
        SM_Logs(LOG_ERROR,_RRC_,"Invalid TMSI size");
        return;
      }
...
static void rrc_gNB_generate_UECapabilityEnquiry(gNB_RRC_INST *rrc, gNB_RRC_UE_t *ue)
{
  ...
  // LOG_I(NR_RRC, "UE %d: Logical Channel DL-DCCH, Generate NR UECapabilityEnquiry (bytes %d, xid %d)", ue->rrc_ue_id, size, xid);
...
static int rrc_gNB_decode_dcch(gNB_RRC_INST *rrc, const f1ap_ul_rrc_message_t *msg)
{
  ...
  if (!ue_context_p) {
    // LOG_E(RRC, "could not find UE context for CU UE ID %u, aborting transaction", msg->gNB_CU_ue_id);
    return -1;
  }
  gNB_RRC_UE_t *UE = &ue_context_p->ue_context;

  if (msg->srb_id < 1 || msg->srb_id > 2) {
    LOG_E(NR_RRC, "Received message on SRB %d, discarding message", msg->srb_id);
    return -1;
  }
  ...
  if ((dec_rval.code != RC_OK) && (dec_rval.consumed == 0)) {
    // LOG_E(NR_RRC, "UE %d: Failed to decode UL-DCCH (%zu bytes)", UE->rrc_ue_id, dec_rval.consumed);
    SM_Logs(LOG_ERROR,_RRC_,"Failed to decode UL-DCCH");
    return -1;
  }
      ...
      case NR_UL_DCCH_MessageType__c1_PR_NOTHING:
        SM_Logs(LOG_INFO, _RRC_, "Received PR_NOTHING on UL-DCCH-Message");
        break;

      case NR_UL_DCCH_MessageType__c1_PR_rrcReconfigurationComplete:
        SM_Logs(LOG_INFO, _RRC_, "Received RRCReconfigurationComplete");
        handle_rrcReconfigurationComplete(rrc, UE, ul_dcch_msg->message.choice.c1->choice.rrcReconfigurationComplete);
        break;

      case NR_UL_DCCH_MessageType__c1_PR_rrcSetupComplete:
        SM_Logs(LOG_INFO, _RRC_, "Received RRCSetupComplete (RRC_CONNECTED reached)");
      ...
      case NR_UL_DCCH_MessageType__c1_PR_ulInformationTransfer:
        SM_Logs(LOG_INFO, _RRC_, "Received RRC UL Information Transfer");
        rrc_gNB_send_NGAP_UPLINK_NAS(rrc, UE, ul_dcch_msg);
        break;

      case NR_UL_DCCH_MessageType__c1_PR_securityModeComplete:
        // LOG_UE_UL_EVENT(UE, "Received Security Mode Complete");
        SM_Logs(LOG_INFO, _RRC_, "Received Security Mode Complete");
        /* configure ciphering */
      ...
      case NR_UL_DCCH_MessageType__c1_PR_rrcReestablishmentComplete:
        ...
        SM_Logs(LOG_INFO, _RRC_, "Received RRCReestablishmentComplete");
...
void rrc_gNB_process_initial_ul_rrc_message(sctp_assoc_t assoc_id, const f1ap_initial_ul_rrc_message_t *ul_rrc)
{
  ...
  gNB_RRC_INST *rrc = RC.nrrrc[0];
  SM_Logs(LOG_INFO, _RRC_, "Decoding CCCH: RNTI %04x, payload_size %d\n", ul_rrc->crnti, ul_rrc->rrc_container_length);
  ...
  if (dec_rval.code != RC_OK || dec_rval.consumed == 0) {
    ...
    SM_Logs(LOG_ERROR,_RRC_,"Failed to decode CCCH");
    return;
  }
...
void *rrc_gnb_task(void *args_p) {
  ...
  SM_Logs(LOG_INFO, _RRC_, "Initiating primary execution loop for handling of the task of RRC ");

//madhav yaah logs add karunga mai
  while (1) {
    ...
    switch (ITTI_MSG_ID(msg_p)) {
      ...
      case NGAP_DOWNLINK_NAS:
        SM_Logs(LOG_INFO, _RRC_, "RRC Layer received NGAP Downlink Msg");
        rrc_gNB_process_NGAP_DOWNLINK_NAS(msg_p, instance, &rrc_gNB_mui);
        break;

      case NGAP_PDUSESSION_SETUP_REQ:
        SM_Logs(LOG_INFO, _RRC_, "RRC Layer received NGAP PDU Session Setup Req");
      ...
      case F1AP_UE_CONTEXT_SETUP_RESP:
        SM_Logs(LOG_INFO, _RRC_, "RRC Layer received F1AP UE Context Setup Response");
        rrc_CU_process_ue_context_setup_response(msg_p, instance);
        break;

      case F1AP_UE_CONTEXT_MODIFICATION_RESP:
        SM_Logs(LOG_INFO, _RRC_, "RRC Layer received F1AP UE Context Modification Response");
      ...
      case F1AP_UE_CONTEXT_RELEASE_REQ:
        SM_Logs(LOG_INFO, _RRC_, "RRC Layer received UE Context Release Request");
      ...
      default:
        ...
        SM_Logs(LOG_ERROR,_RRC_,"Received unknown message");
        break;
    }
...
void rrc_gNB_generate_RRCRelease(gNB_RRC_INST *rrc, gNB_RRC_UE_t *UE)
{
  ...
  SM_Logs(LOG_INFO,_RRC_,"Generating UE Release for UE[%d]",UE->rrc_ue_id);
  f1_ue_data_t ue_data = cu_get_f1_ue_data(UE->rrc_ue_id);
  RETURN_IF_INVALID_ASSOC_ID(ue_data.du_assoc_id);
...
//-----------------------------------------------------------------------------
{
  SM_Logs(LOG_INFO, _RRC_, "Generating UE Context Setup Request");
  ...
  SM_Logs(LOG_INFO, _RRC_, "UE Id [%d] SRBs [%d] DRBs [%d]",ue_p->rrc_ue_id,nb_srb,n_drbs );
