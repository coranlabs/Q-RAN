...
{
  (void)reset;
  SM_Logs(LOG_ERROR, _RRC_, "%s() not implemented yet\n", __func__);
}
...
static void ue_context_setup_request_f1ap(sctp_assoc_t assoc_id, const f1ap_ue_context_setup_t *req)
{
  ...
  *f1ap_msg = *req;
  if (req->cu_to_du_rrc_information != NULL) {
    ...
    if (f1ap_msg->cu_to_du_rrc_information == NULL) {
    SM_Logs(LOG_ERROR, _F1AP_, "Out of memory\n");
    return;
    }

    if (req->cu_to_du_rrc_information->cG_ConfigInfo != NULL || req->cu_to_du_rrc_information->cG_ConfigInfo_length != 0) {
        SM_Logs(LOG_ERROR, _F1AP_, "cg_ConfigInfo not implemented\n");
        return;
    }

    if (req->cu_to_du_rrc_information->measConfig != NULL || req->cu_to_du_rrc_information->measConfig_length != 0) {
        SM_Logs(LOG_ERROR, _F1AP_, "measConfig not implemented\n");
        return;
    }
    ...
    if (req->cu_to_du_rrc_information->uE_CapabilityRAT_ContainerList != NULL) {
     ...
     if (du2cu_req->uE_CapabilityRAT_ContainerList_length <= 0) {
          SM_Logs(LOG_ERROR, _RRC_, "uE_CapabilityRAT_ContainerList_length is invalid (<= 0)\n");
          return;
      }
      ...
      if (du2cu_new->uE_CapabilityRAT_ContainerList == NULL) {
    SM_Logs(LOG_ERROR, _RRC_, "uE_CapabilityRAT_ContainerList is NULL (out of memory)\n");
    return;
}
    ...
    if (req->cu_to_du_rrc_information->handoverPreparationInfo != NULL) {
      cu_to_du_rrc_information_t *du2cu_new = f1ap_msg->cu_to_du_rrc_information;
              if (du2cu_req->handoverPreparationInfo_length <= 0) {
            SM_Logs(LOG_ERROR, _RRC_, "handoverPreparationInfo_length is not greater than 0\n");
            return;
        }
  ...
  if (req->drbs_to_be_setup_length > 0) {
    ...
    f1ap_msg->drbs_to_be_setup = calloc(n, sizeof(*f1ap_msg->drbs_to_be_setup));
        if (f1ap_msg->drbs_to_be_setup == NULL) {
        SM_Logs(LOG_ERROR, _RRC_, "drbs_to_be_setup is NULL, out of memory\n");
        return;
    }
  ...
  if (req->srbs_to_be_setup_length > 0) {
    ...
    f1ap_msg->srbs_to_be_setup = calloc(n, sizeof(*f1ap_msg->srbs_to_be_setup));
        if (f1ap_msg->srbs_to_be_setup == NULL) {
        SM_Logs(LOG_ERROR, _RRC_, "srbs_to_be_setup is NULL, out of memory\n");
        return;
    }
  ...
  if (req->rrc_container_length > 0) {
    ...
    if (f1ap_msg->rrc_container == NULL) {
    SM_Logs(LOG_ERROR, _F1AP_, "rrc_container is NULL, out of memory\n");
    return;
}
...
static void ue_context_modification_request_f1ap(sctp_assoc_t assoc_id, const f1ap_ue_context_modif_req_t *req)
{
  ...
  *f1ap_msg = *req;
  if (req->cu_to_du_rrc_information != NULL) {
    f1ap_msg->cu_to_du_rrc_information = calloc(1, sizeof(*f1ap_msg->cu_to_du_rrc_information));
    if (f1ap_msg->cu_to_du_rrc_information == NULL) {
    SM_Logs(LOG_ERROR, _F1AP_, "cu_to_du_rrc_information is NULL, out of memory\n");
    return;
    }

    if (req->cu_to_du_rrc_information->cG_ConfigInfo != NULL || req->cu_to_du_rrc_information->cG_ConfigInfo_length != 0) {
        SM_Logs(LOG_ERROR, _F1AP_, "cG_ConfigInfo is not implemented\n");
        return;
    }

    if (req->cu_to_du_rrc_information->measConfig != NULL || req->cu_to_du_rrc_information->measConfig_length != 0) {
        SM_Logs(LOG_ERROR, _F1AP_, "measConfig is not implemented\n");
        return;
    }
  ...
  SM_Log_Assert(_ARIA_,req->drbs_to_be_modified_length == 0, "Updating DRBs is not supported ");
    if (req->drbs_to_be_setup_length > 0) {
      int n = req->drbs_to_be_setup_length;
      f1ap_msg->drbs_to_be_setup_length = n;
      f1ap_msg->drbs_to_be_setup = calloc(n, sizeof(*f1ap_msg->drbs_to_be_setup));
    ...
    if (req->drbs_to_be_released_length > 0) {
      ...
      if (f1ap_msg->drbs_to_be_released == NULL) {
        SM_Logs(LOG_ERROR, _F1AP_, "out of memory\n");
        return;
      }
    ...
    if (req->transm_action_ind != NULL) {
      ...
      if (f1ap_msg->transm_action_ind == NULL) {
        SM_Logs(LOG_ERROR, _F1AP_, "out of memory\n");
        return;
      }
