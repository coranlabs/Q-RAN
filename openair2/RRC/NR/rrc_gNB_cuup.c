...
int rrc_gNB_process_e1_setup_req(sctp_assoc_t assoc_id, e1ap_setup_req_t *req)
{
  ...
  for (int i = 0; i < req->supported_plmns; i++) {
    ...
    if (rrc->configuration.mcc[i] != id->mcc || rrc->configuration.mnc[i] != id->mnc) {
      ...
      SM_Logs(LOG_ERROR,_RRC_,"Plmn provided by CUUP (mcc:%d, mnc:%d) don't align with the RRC configurations (mcc:%d, mnc:%d)\n",id->mcc,id->mnc,rrc->configuration.mcc[i],rrc->configuration.mnc[i]);
  ...
  SM_Logs(LOG_INFO, _RRC_, " Connection Protocol: Acknowledging and securing new Central Unit User Plane (CU-UP) session. Initializing handshake and session parameters");
