...
int e1ap_decode_initiating_message(E1AP_E1AP_PDU_t *pdu) {
  if (pdu == NULL) {
    SM_Logs(LOG_ERROR, _E1AP_, "The PDU pointer is NULL. This error must be resolved to proceed.\n");
    return; // Replace with appropriate error handling logic as needed.
}
  ...
  switch(pdu->choice.initiatingMessage->procedureCode) {
    ...
    default:
     SM_Logs(LOG_ERROR, _E1AP_, "Procedure code (%d) for the initiating message is not supported.\n",
...
int e1ap_decode_successful_outcome(E1AP_E1AP_PDU_t *pdu) {
  ...
  switch(pdu->choice.successfulOutcome->procedureCode) {
    ...
    default:
      SM_Logs(LOG_ERROR, _E1AP_, "The procedure code (%d) for the successful message is not supported.\n",
...
int e1ap_decode_unsuccessful_outcome(E1AP_E1AP_PDU_t *pdu) {
  ...
  switch(pdu->choice.unsuccessfulOutcome->procedureCode) {
    ...
    default:
    SM_Logs(LOG_ERROR, _E1AP_, "The procedure code (%d) for the unsuccessful message is not supported.\n",
...
int e1ap_decode_pdu(E1AP_E1AP_PDU_t *pdu, const uint8_t *const buffer, uint32_t length) {
  ...
  if (buffer == NULL) {
    SM_Logs(LOG_ERROR, _E1AP_, "Buffer is NULL. This condition must be addressed to avoid unexpected behavior.\n");
  ...
  switch(pdu->present) {
    ...
    default:
      SM_Logs(LOG_ERROR, _E1AP_, "The presence (%d) is unknown or not implemented.\n", (int)pdu->present);
...
{
  if (pdu == NULL) {
    SM_Logs(LOG_ERROR, _E1AP_, "PDU is NULL. This must be addressed to ensure proper functionality.\n");
    return; // or handle the error appropriately depending on your application's requirements
  }
