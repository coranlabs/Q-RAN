...
{
  SM_Logs(LOG_INFO, _F1AP_, "Handling DL RRC Message Transfer.");
  f1ap_dl_rrc_message_t msg = {0};
  if (!decode_dl_rrc_message_transfer(pdu, &msg)) {
    // LOG_E(F1AP, "cannot decode F1 DL RRC message Transfer\n");
    SM_Logs(LOG_ERROR, _F1AP_, "Failed to decode DL RRC Message Transfer.");
    free_dl_rrc_message_transfer(&msg);
    return -1;
  }

  SM_Logs(LOG_DEBUG, _F1AP_, "DL RRC Message Transfer decoded successfully.");
  SM_Logs(LOG_DEBUG, _F1AP_, "Message Details: UE RNTI = %x, SRB ID = %d, RRC Container Length = %d.", 
          msg.gNB_DU_ue_id, msg.srb_id, msg.rrc_container_length);
...
int DU_send_INITIAL_UL_RRC_MESSAGE_TRANSFER(sctp_assoc_t assoc_id, const f1ap_initial_ul_rrc_message_t *msg)
{
  ...
  if (pdu == NULL) {
    ...
    SM_Logs(LOG_ERROR, _F1AP_, "Failed to encode INITIAL UL RRC Message Transfer. Cannot proceed with sending.");
  ...
  if (f1ap_encode_pdu(pdu, &buffer, &len) < 0) {
    ...
    SM_Logs(LOG_ERROR,_F1AP_,"Error while encoding Initial UL RRC Message Transfer"); 
    return -1;
  }
  ...
  /* Send to ITTI */
  SM_Logs(LOG_DEBUG,_F1AP_,"Forwarding Initial UL RRC Message Transfer to SCTP Task.");
  f1ap_itti_send_sctp_data_req(assoc_id, buffer, len);
  SM_Logs(LOG_INFO, _F1AP_, "Initial UL RRC Message Transfer Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
  return 0;
}
...
int DU_send_UL_NR_RRC_MESSAGE_TRANSFER(sctp_assoc_t assoc_id, const f1ap_ul_rrc_message_t *msg)
{
  ...
  if (pdu == NULL) {
    ...
    SM_Logs(LOG_ERROR, _F1AP_, "Failed to encode UL NR RRC Message Transfer. Cannot proceed with sending.");
    ASN_STRUCT_FREE(asn_DEF_F1AP_F1AP_PDU, pdu);
    return -1;
  }
  /* encode F1AP PDU */
  ...
  if (f1ap_encode_pdu(pdu, &buffer, &len) < 0) {
    ...
    SM_Logs(LOG_ERROR,_F1AP_,"Error while encoding  UL NR RRC Message Transfer");
    ASN_STRUCT_FREE(asn_DEF_F1AP_F1AP_PDU, pdu);
    return -1;
  }
  /* Free F1AP PDU */
  ...
  SM_Logs(LOG_DEBUG,_F1AP_,"Forwarding UL NR RRC Message Transfer to SCTP Task.");
  /* Send to ITTI */
  f1ap_itti_send_sctp_data_req(assoc_id, buffer, len);
  SM_Logs(LOG_INFO, _F1AP_, " UL NR RRC Message Transfer Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
  return 0;
}
