...
{
  SM_Logs(LOG_INFO, _F1AP_, "Processing UE Context Setup Request received from the Central Unit (CU).");
  ...
  SM_Logs(LOG_DEBUG, _F1AP_, "Decoding service cell and UL configuration details.");
  /* SpCell_ID */
  ...
  SM_Logs(LOG_DEBUG, _F1AP_, "Processing RRC information and bearer details.");
...
{
  SM_Logs(LOG_INFO, _F1AP_, "Initiating the transmission of UE Context Setup Response to the Central Unit (CU).");
  ...
  if (f1ap_encode_pdu(&pdu, &buffer, &len) < 0) {
    ...
    SM_Logs(LOG_ERROR,_F1AP_,"Error while encoding UE Context Setup Response");

    return -1;
  }
  SM_Logs_Buffer(LOG_DEBUG, _F1AP_, "Encoded UE Context Setup Response Buffer", buffer, len);
  SM_Logs(LOG_DEBUG,_F1AP_,"Forwarding UE Context Setup Response to SCTP Task.");
  f1ap_itti_send_sctp_data_req(assoc_id, buffer, len);
  SM_Logs(LOG_INFO, _F1AP_, "UE Context Setup Response Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
  return 0;
}
...
{
  SM_Logs(LOG_INFO, _F1AP_, "Initiating the transmission of UE Context Release Request to the Central Unit (CU).");
  ...
  if (f1ap_encode_pdu(&pdu, &buffer, &len) < 0) {
    ...
    SM_Logs(LOG_ERROR,_F1AP_,"Error while encoding UE Context Release Request");
    return -1;
  }

  SM_Logs(LOG_DEBUG,_F1AP_,"Forwarding UE Context Release Request to SCTP Task.");
  f1ap_itti_send_sctp_data_req(assoc_id, buffer, len);
  SM_Logs(LOG_INFO, _F1AP_, "UE Context Release Request Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
  return 0;
}

int DU_handle_UE_CONTEXT_RELEASE_COMMAND(instance_t instance, sctp_assoc_t assoc_id, uint32_t stream, F1AP_F1AP_PDU_t *pdu)
{
  SM_Logs(LOG_INFO, _F1AP_, "Processing UE Context Release Command received from the Central Unit (CU).");
  ...
  if (ie != NULL) {
    ...
    SM_Log_Assert(_F1AP_, f1ap_ue_context_release_cmd->rrc_container != NULL, "Memory allocation failed for RRC Container.");
  ...
  case F1AP_Cause_PR_misc:
    ...
    break;
  default:
    SM_Logs(LOG_WARN, _F1AP_, "Unrecognized cause received for UE context release.");
...
{
  SM_Logs(LOG_INFO, _F1AP_, "Initiating the UE Context Release Complete transmission to the Central Unit (CU).");
  ...
  if (f1ap_encode_pdu(&pdu, &buffer, &len) < 0) {
    // LOG_E(F1AP, "Failed to encode F1ResetAcknowledge\n");
    SM_Logs(LOG_ERROR,_F1AP_,"While encoding UE Context Release Complete");
    return -1;
  }

  SM_Logs(LOG_DEBUG,_F1AP_,"Forwarding UE Context Release Complete to SCTP Task.");
  f1ap_itti_send_sctp_data_req(assoc_id, buffer, len);
  SM_Logs(LOG_INFO, _F1AP_, "UE Context Release Complete Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
  return 0;
}
...
{
  SM_Logs(LOG_INFO, _F1AP_, "Processing UE Context Modification Request received from the Central Unit (CU).");
  ...
  if(ieSrb != NULL) {
    ...
    //     "could not allocate memory for f1ap_ue_context_setup_req->srbs_to_be_setup\n");
    SM_Log_Assert(_F1AP_, f1ap_ue_context_modification_req->srbs_to_be_setup != NULL,"Memory allocation failed for SRBs to be setup/modified.");
  ...
  if(ieDrb!=NULL) {
    ...
    SM_Log_Assert(_F1AP_, f1ap_ue_context_modification_req->drbs_to_be_setup != NULL, "Memory allocation failed for DRBs to be setup/modified.");
  ...
  if (ieDrb != NULL) {
    ...
    SM_Log_Assert(_F1AP_, f1ap_ue_context_modification_req->drbs_to_be_released != NULL, "Memory allocation failed for DRBs to be released.");
...
{
  SM_Logs(LOG_INFO, _F1AP_, "Initiating the UE Context Modification Response transmission to the Central Unit (CU).");
  F1AP_F1AP_PDU_t                        pdu= {0};
  ...
  if (f1ap_encode_pdu(&pdu, &buffer, &len) < 0) {
    // LOG_E(F1AP, "Failed to encode F1ResetAcknowledge\n");
    SM_Logs(LOG_ERROR,_F1AP_,"Error while encoding UE Context Modification Response");
    return -1;
  }

  SM_Logs(LOG_DEBUG,_F1AP_,"Forwarding UE Context Modification Response to SCTP Task.");
  f1ap_itti_send_sctp_data_req(assoc_id, buffer, len);
  SM_Logs(LOG_INFO, _F1AP_, "UE Context Modification Response Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
  return 0;
}
...
int DU_send_UE_CONTEXT_MODIFICATION_REQUIRED(sctp_assoc_t assoc_id, f1ap_ue_context_modif_required_t *required)
{
  ...
  if (f1ap_encode_pdu(&pdu, &buffer, &len) < 0) {
    // LOG_E(F1AP, "Failed to encode F1ResetAcknowledge\n");
    SM_Logs(LOG_ERROR,_F1AP_,"Error while encoding UE Context Modification Required");
    return -1;
  }

  SM_Logs(LOG_DEBUG,_F1AP_,"Forwarding UE Context Modification Required to SCTP Task.");
  f1ap_itti_send_sctp_data_req(assoc_id, buffer, len);
  SM_Logs(LOG_INFO, _F1AP_, "UE Context Modification Required Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
  return 0;
}
