...
void du_task_send_ssl_handshake_req(instance_t instance, sctp_assoc_t assoc_id)
{
  MessageDef *message_p = NULL;
  ...
  message_p = itti_alloc_new_message(TASK_DU_F1, 0, SCTP_SSL_DO_HANDSHAKE);
...
void du_task_handle_sctp_association_resp(instance_t instance, sctp_new_association_resp_t *sctp_new_association_resp)
{
  ...
  sleep(1.5);
  du_task_send_ssl_handshake_req(instance, f1ap_du_data->du.assoc_id);
  sleep(2.0);
  ...
  while (1) {
    ...
    switch (ITTI_MSG_ID(msg)) {
      ...
      case F1AP_GNB_CU_CONFIGURATION_UPDATE_ACKNOWLEDGE:
        DU_send_gNB_CU_CONFIGURATION_UPDATE_ACKNOWLEDGE(assoc_id, &F1AP_GNB_CU_CONFIGURATION_UPDATE_ACKNOWLEDGE(msg));
        break;

      case F1AP_GNB_CU_CONFIGURATION_UPDATE_FAILURE:
        DU_send_gNB_CU_CONFIGURATION_UPDATE_FAILURE(assoc_id, &F1AP_GNB_CU_CONFIGURATION_UPDATE_FAILURE(msg));
        break;

      case SCTP_NEW_ASSOCIATION_RESP:
        // 1. store the respon
        // 2. send the f1setup_req
        du_task_handle_sctp_association_resp(myInstance, &msg->ittiMsg.sctp_new_association_resp);
        break;

      case SCTP_DATA_IND:
        // ex: any F1 incoming message for DU ends here
        du_task_handle_sctp_data_ind(myInstance, &msg->ittiMsg.sctp_data_ind);
        break;
      ...
      case F1AP_UE_CONTEXT_RELEASE_REQ: // from MAC
        DU_send_UE_CONTEXT_RELEASE_REQUEST(assoc_id, &F1AP_UE_CONTEXT_RELEASE_REQ(msg));
        break;
