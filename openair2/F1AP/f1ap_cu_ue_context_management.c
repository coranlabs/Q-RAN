...
int CU_send_UE_CONTEXT_SETUP_REQUEST(sctp_assoc_t assoc_id, f1ap_ue_context_setup_t *f1ap_ue_context_setup_req)
{
  ...
  if (f1ap_encode_pdu(&pdu, &buffer, &len) < 0) {
    ...
    SM_Logs(LOG_ERROR,_F1AP_,"Error while encoding UE Context Setup Request");
    return -1;
  }
  SM_Logs(LOG_DEBUG,_F1AP_,"UE Context Setup Request encoding \033[0;32mSuccessful\033[0m");
  ...
  SM_Logs(LOG_DEBUG,_F1AP_,"Forwarding UE Context Setup Request  to SCTP Task.");
  f1ap_itti_send_sctp_data_req(assoc_id, buffer, len);
  SM_Logs(LOG_INFO, _F1AP_, "UE Context Setup Request  Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
  return 0;
}
...
{
  SM_Logs(LOG_INFO, _F1AP_, "Processing UE Context Release Request received from the Distributed Unit (DU).");
  ...
  SM_Logs(LOG_DEBUG, _F1AP_, "Decoded UE identifiers successfully. Processing release cause.");
  ...
  switch(ie->value.choice.Cause.present)
  {
    case F1AP_Cause_PR_radioNetwork:
      ...
      SM_Logs(LOG_DEBUG, _F1AP_, "Release caused by radio network issues.");
      break;
    case F1AP_Cause_PR_transport:
      req->cause = F1AP_CAUSE_TRANSPORT;
      req->cause_value = ie->value.choice.Cause.choice.transport;
      SM_Logs(LOG_DEBUG, _F1AP_, "Release caused by transport-related issues.");
      break;
    case F1AP_Cause_PR_protocol:
      req->cause = F1AP_CAUSE_PROTOCOL;
      req->cause_value = ie->value.choice.Cause.choice.protocol;
      SM_Logs(LOG_DEBUG, _F1AP_, "Release caused by protocol-related issues.");
      break;
    case F1AP_Cause_PR_misc:
      req->cause = F1AP_CAUSE_MISC;
      req->cause_value = ie->value.choice.Cause.choice.misc;
      SM_Logs(LOG_DEBUG, _F1AP_, "Release caused by miscellaneous issues.");
      break;
...
{
  SM_Logs(LOG_INFO, _F1AP_, "Initiating transmission of UE Context Release Command to the Distributed Unit (DU).");
  F1AP_F1AP_PDU_t                   pdu= {0};
  ...
  if (f1ap_encode_pdu(&pdu, &buffer, &len) < 0) {
    ...
    SM_Logs(LOG_ERROR,_F1AP_,"Error while encoding UE Context Release Command");
    return -1;
  }

  SM_Logs(LOG_DEBUG,_F1AP_,"Forwarding UE Context Release Command  to SCTP Task.");
  f1ap_itti_send_sctp_data_req(assoc_id, buffer, len);
  SM_Logs(LOG_INFO, _F1AP_, "UE Context Release Command  Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
  return 0;
}
...
int CU_send_UE_CONTEXT_MODIFICATION_REQUEST(sctp_assoc_t assoc_id, f1ap_ue_context_modif_req_t *f1ap_ue_context_modification_req)
{
  ...
  if (f1ap_encode_pdu(&pdu, &buffer, &len) < 0) {
    ...
    SM_Logs(LOG_ERROR,_F1AP_,"Error while encoding UE Context Modification Request");
    return -1;
  }
  SM_Logs(LOG_DEBUG,_F1AP_,"Forwarding UE Context Modification Request  to SCTP Task.");
  f1ap_itti_send_sctp_data_req(assoc_id, buffer, len);
  SM_Logs(LOG_INFO, _F1AP_, "UE Context Modification Request  Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
  return 0;
}
...
int CU_send_UE_CONTEXT_MODIFICATION_CONFIRM(sctp_assoc_t assoc_id, f1ap_ue_context_modif_confirm_t *confirm)
{
  ...
  if (f1ap_encode_pdu(&pdu, &buffer, &len) < 0) {
    // LOG_E(F1AP, "Failed to encode F1 context release command\n");
    SM_Logs(LOG_ERROR,_F1AP_,"Error while encoding UE Context Modification Confirm");
    return -1;
  }

  SM_Logs(LOG_DEBUG,_F1AP_,"ForwardingUE Context Modification Confirm  to SCTP Task.");
  f1ap_itti_send_sctp_data_req(assoc_id, buffer, len);
  SM_Logs(LOG_INFO, _F1AP_, "UE Context Modification Confirm Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
  return 0;
}
...
int CU_send_UE_CONTEXT_MODIFICATION_REFUSE(sctp_assoc_t assoc_id, f1ap_ue_context_modif_refuse_t *refuse)
{
  ...
  if (f1ap_encode_pdu(&pdu, &buffer, &len) < 0) {
    // LOG_E(F1AP, "Failed to encode F1 context release command\n");
    SM_Logs(LOG_ERROR,_F1AP_,"Error while encoding UE Context Modification Refuse");
    return -1;
  }

  SM_Logs(LOG_DEBUG,_F1AP_,"Forwarding UE Context Modification Refuse to SCTP Task.");
  f1ap_itti_send_sctp_data_req(assoc_id, buffer, len);
  SM_Logs(LOG_INFO, _F1AP_, "UE Context Modification Refuse Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
  return 0;
}
