...
int CU_send_Paging(sctp_assoc_t assoc_id, f1ap_paging_ind_t *paging)
{
  ...
  if (f1ap_encode_pdu(&pdu, &buffer, &len) < 0) {
    ...
    SM_Logs(LOG_ERROR,_F1AP_,"Error while encoding Paging Message");
    ASN_STRUCT_FREE_CONTENTS_ONLY(asn_DEF_F1AP_F1AP_PDU, &pdu);
    return -1;
  }
  SM_Logs(LOG_DEBUG,_F1AP_,"Paging Message encoding \033[0;32mSuccessful\033[0m ");
  SM_Logs(LOG_DEBUG,_F1AP_,"Forwarding Paging Message to SCTP Task.");

  ASN_STRUCT_FREE_CONTENTS_ONLY(asn_DEF_F1AP_F1AP_PDU, &pdu);
  f1ap_itti_send_sctp_data_req(assoc_id, buffer, len);
  SM_Logs(LOG_INFO, _F1AP_, "Paging Message Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
  return 0;
}
