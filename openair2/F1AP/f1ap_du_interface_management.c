...
int DU_send_RESET_ACKNOWLEDGE(sctp_assoc_t assoc_id, const f1ap_reset_ack_t *ack)
{
  ...
  if (f1ap_encode_pdu(&pdu, &buffer, &len) < 0) {
    ...
    SM_Logs(LOG_ERROR,_F1AP_,"Error while encoding Reset Acknowledge");
    return -1;
  }

  /* send */
  ASN_STRUCT_RESET(asn_DEF_F1AP_F1AP_PDU, &pdu);
  SM_Logs(LOG_DEBUG,_F1AP_,"Forwarding Reset Acknowledge to SCTP Task.");
  f1ap_itti_send_sctp_data_req(assoc_id, buffer, len);
  SM_Logs(LOG_INFO, _F1AP_, "Reset Acknowledge Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
  return 0;
}
...
{
  SM_Logs(LOG_INFO, _F1AP_, "Initiating the F1AP Setup Request transmission to establish communication with the Central Unit (CU).");
  ...
  /* encode */
  if (f1ap_encode_pdu(pdu, &buffer, &len) < 0) {
    SM_Logs(LOG_ERROR,_F1AP_,"Error while encoding F1AP Setup Request");
  ...
  /* Log the encoded buffer content in DEBUG mode */
  SM_Logs_Buffer(LOG_DEBUG, _F1AP_, "Encoded F1AP Setup Request Buffer", buffer, len);


  /* free PDU */
  ASN_STRUCT_FREE(asn_DEF_F1AP_F1AP_PDU, pdu);
  /* Send to ITTI */
  SM_Logs(LOG_DEBUG,_F1AP_,"Forwarding F1AP Setup Request to SCTP Task.");
  f1ap_itti_send_sctp_data_req(assoc_id, buffer, len);
  SM_Logs(LOG_INFO, _F1AP_, "F1AP Setup Request Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
  return 0;
}

int DU_handle_F1_SETUP_RESPONSE(instance_t instance, sctp_assoc_t assoc_id, uint32_t stream, F1AP_F1AP_PDU_t *pdu)
{
  SM_Logs(LOG_INFO, _F1AP_, "Handling F1 Setup Response.");
  // LOG_D(F1AP, "DU_handle_F1_SETUP_RESPONSE\n");
  /* Decode */
  f1ap_setup_resp_t resp = {0};
  if (!decode_f1ap_setup_response(pdu, &resp)) {
    SM_Logs(LOG_ERROR, _F1AP_, "Failed to decode F1AP Setup Response.");
    // LOG_E(F1AP, "cannot decode F1AP Setup Response\n");
    free_f1ap_setup_response(&resp);
    return -1;
  }

  SM_Logs(LOG_INFO, _F1AP_, "Successfully decoded F1 Setup Response, Transaction ID: %lu, gNB CU Name: %s, Number of cells to activate: %d, RRC Version: %d.%d.%d.", resp.transaction_id, resp.gNB_CU_name ? resp.gNB_CU_name : "N/A", resp.num_cells_to_activate,resp.rrc_ver[0], resp.rrc_ver[1], resp.rrc_ver[2]);
...
{
  SM_Logs(LOG_INFO, _F1AP_, "Initiating the GNB DU Configuration Update transmission to the Central Unit (CU).");
  uint8_t  *buffer=NULL;
  uint32_t  len=0;
  /* encode F1AP message */
  ...
  if (f1ap_encode_pdu(pdu, &buffer, &len) < 0) {
    ...
    SM_Logs(LOG_ERROR,_F1AP_,"Error while encoding GNB DU Configuration Update");
    return -1;
  }
  SM_Logs_Buffer(LOG_DEBUG, _F1AP_, "Encoded GNB DU Configuration Update Buffer", buffer, len);

  /* transfer the message */
  ASN_STRUCT_FREE(asn_DEF_F1AP_F1AP_PDU, pdu);
  SM_Logs(LOG_DEBUG,_F1AP_,"Forwarding GNB DU Configuration Update to SCTP Task.");
  f1ap_itti_send_sctp_data_req(assoc_id, buffer, len);
  SM_Logs(LOG_INFO, _F1AP_, "GNB DU Configuration Update Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
  return 0;
}
...
{
  SM_Logs(LOG_INFO, _F1AP_, "Processing the GNB CU Configuration Update received from the Central Unit (CU).");
  ...
  if (!decode_f1ap_cu_configuration_update(pdu, &in)) {
    SM_Logs(LOG_ERROR, _F1AP_, "Decoding of the GNB CU Configuration Update failed. Unable to proceed with the update.");
    // LOG_E(F1AP, "Failed to decode F1AP Setup Failure\n");
  ...
  *msg = in; // copy F1 message to ITTI
  ...
  SM_Logs(LOG_DEBUG, _F1AP_, "gNB-CU Configuration Update successfully decoded. Transferring the message to GNB APP Task.");
...
int DU_handle_gNB_DU_CONFIGURATION_UPDATE_ACKNOWLEDGE(instance_t instance,
                                                      ...
                                                      F1AP_F1AP_PDU_t *pdu)
{
  SM_Logs(LOG_INFO, _F1AP_, "Processing gNB DU Configuration Update Acknowledge received from the Central Unit (CU).");
  // Decoding
  ...
  if (!decode_f1ap_du_configuration_update_acknowledge(pdu, &in)) {
    ...
    SM_Logs(LOG_ERROR, _F1AP_, "Decoding of the GNB DU Configuration Update Acknowledge failed. Unable to proceed.");
  ...
  *msg = in; // Copy the decoded message to the ITTI message (RRC thread will free it)
  SM_Logs(LOG_DEBUG, _F1AP_, "Decoded GNB DU Configuration Update Acknowledge successfully. Forwarding the message to GNB App Task for processing.");
  itti_send_msg_to_task(TASK_GNB_APP, GNB_MODULE_ID_TO_INSTANCE(assoc_id), msg_p);
  return 0;
}
...
{
  SM_Logs(LOG_INFO, _F1AP_, "Initiating the transmission of GNB CU Configuration Update Acknowledge to the Central Unit (CU).");
  uint8_t *buffer = NULL;
  uint32_t len = 0;
  /* encode F1 message */
  ...
  if (!pdu || f1ap_encode_pdu(pdu, &buffer, &len) < 0) {
    ...
    SM_Logs(LOG_ERROR,_F1AP_,"Error while encoding GNB CU Configuration Update Acknowledge");
    ASN_STRUCT_FREE(asn_DEF_F1AP_F1AP_PDU, pdu);
    return -1;
  }
  SM_Logs_Buffer(LOG_DEBUG, _F1AP_, "Encoded GNB CU Configuration Update Acknowledge Buffer", buffer, len);
  ASN_STRUCT_FREE(asn_DEF_F1AP_F1AP_PDU, pdu);
  SM_Logs(LOG_DEBUG,_F1AP_,"Forwarding GNB CU Configuration Update Acknowledge to SCTP Task.");
  f1ap_itti_send_sctp_data_req(assoc_id, buffer, len);
  SM_Logs(LOG_INFO, _F1AP_, "GNB CU Configuration Update Acknowledge Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
  return 0;
}
