...
size_t dump_mac_stats(gNB_MAC_INST *gNB, char *output, size_t strlen, bool reset_rsrp)
{
  ...
  //madhav changes for adding ue table.
  int iterator = 0;
  int num_UEs = 0;

  UE_iterator(gNB->UE_info.list, UE_2) {
    num_UEs+=1;
  }

  // printf("\nTESTING --> num of UE : %d\n",num_UEs);
  aria_ue_stats.num_of_ue = num_UEs;

  UE_iterator(gNB->UE_info.list, UE) {
    NR_UE_sched_ctrl_t *sched_ctrl = &UE->UE_sched_ctrl;
    NR_mac_stats_t *stats = &UE->mac_stats;
    const int avg_rsrp = stats->num_rsrp_meas > 0 ? stats->cumul_rsrp / stats->num_rsrp_meas : 0;
    aria_ue_stats.stats[iterator].ue_rnti = UE->rnti;
    aria_ue_stats.stats[iterator].avg_rsrp = avg_rsrp;
    ...
    if (du_exists_f1_ue_data(UE->rnti)) {
      ...
      aria_ue_stats.stats[iterator].cu_ue_id = ued.secondary_ue;
    } else {
      output += snprintf(output, end-output, "(none)");
      aria_ue_stats.stats[iterator].cu_ue_id = NULL;

    }

    bool in_sync = !sched_ctrl->ul_failure;
    aria_ue_stats.stats[iterator].ph = sched_ctrl->ph;
    aria_ue_stats.stats[iterator].pcmax = sched_ctrl->pcmax;
    aria_ue_stats.stats[iterator].num_meas = stats->num_rsrp_meas;
    ...
    // printf("\nTESTING --> Ph ; %d\n",  sched_ctrl->ph);
    // printf("\nTESTING --> Ph ; %d\n",  aria_ue_stats.stats[iterator].ph);
    // printf("\nTESTING --> pcmax ; %d\n",  sched_ctrl->pcmax);
    // printf("\nTESTING --> pcmax ; %d\n",  aria_ue_stats.stats[iterator].pcmax);



    if (in_sync)
    {
      strcpy(aria_ue_stats.stats[iterator].sync_status, "\033[0;32mHealthy    \033[0m");
    }else{
      strcpy(aria_ue_stats.stats[iterator].sync_status, "\033[0;31mOut of Sync\033[0m");
    }
    ...
    aria_ue_stats.stats[iterator].dlsch_errors = stats->dl.errors;
    aria_ue_stats.stats[iterator].bler_dlsch = sched_ctrl->dl_bler_stats.bler;
    aria_ue_stats.stats[iterator].pucch0_dtx = stats->pucch0_DTX;
    aria_ue_stats.stats[iterator].mcs_dl = sched_ctrl->dl_bler_stats.mcs;
    aria_ue_stats.stats[iterator].dl_total_bytes = stats->dl.total_bytes;
    aria_ue_stats.stats[iterator].dl_current_bytes = stats->dl.current_bytes;
    aria_ue_stats.stats[iterator].dl_total_rbs = stats->dl.total_rbs;
    aria_ue_stats.stats[iterator].dl_total_rbs_retx = stats->dl.total_rbs_retx;
    aria_ue_stats.stats[iterator].dl_current_rbs = stats->dl.current_rbs;
    aria_ue_stats.stats[iterator].pmi[0] = sched_ctrl->CSI_report.cri_ri_li_pmi_cqi_report.pmi_x1;
    aria_ue_stats.stats[iterator].pmi[1] = sched_ctrl->CSI_report.cri_ri_li_pmi_cqi_report.pmi_x2;
    aria_ue_stats.stats[iterator].cqi = sched_ctrl->CSI_report.cri_ri_li_pmi_cqi_report.wb_cqi_1tb;
    ...
    if (reset_rsrp) {
      ...
      stats->cumul_rsrp = 0;
      aria_ue_stats.stats[iterator].num_meas = 0;
    }
    output += snprintf(output,
                       end - output,
    ...
    aria_ue_stats.stats[iterator].ulsch_errors = stats->ul.errors;
    aria_ue_stats.stats[iterator].bler_ulsch = sched_ctrl->ul_bler_stats.bler;
    aria_ue_stats.stats[iterator].ulsch_dtx = stats->ulsch_DTX;
    aria_ue_stats.stats[iterator].mcs_ul = sched_ctrl->ul_bler_stats.mcs;
    aria_ue_stats.stats[iterator].qm_ul = nr_get_Qm_ul(sched_ctrl->ul_bler_stats.mcs,UE->current_UL_BWP.mcs_table);
    aria_ue_stats.stats[iterator].delta_mcs = UE->mac_stats.deltaMCS;
    aria_ue_stats.stats[iterator].nprb_ul =  UE->mac_stats.NPRB;
    aria_ue_stats.stats[iterator].ul_total_bytes = stats->ul.total_bytes;
    aria_ue_stats.stats[iterator].ul_current_bytes = stats->ul.current_bytes;
    aria_ue_stats.stats[iterator].ul_total_rbs = stats->ul.total_rbs;
    aria_ue_stats.stats[iterator].ul_total_rbs_retx = stats->ul.total_rbs_retx;
    aria_ue_stats.stats[iterator].ul_current_rbs = stats->ul.current_rbs;
    ...
    aria_ue_stats.stats[iterator].mac_tx_bytes = stats->dl.total_bytes;
    aria_ue_stats.stats[iterator].mac_rx_bytes = stats->ul.total_bytes;
                      ...
                      const nr_mac_config_t *config)
{
  ...
  if (RC.nb_nr_macrlc_inst > 0) {
    ...
    SM_Log_Assert(_RLC_, rlc_module_init(1) == 0,"cannot initialize RLC layer");
