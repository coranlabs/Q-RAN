...
static  int udpServerSocket(openAddr_s addr) {
  // LOG_I(GTPU, "Initializing UDP for local address %s with port %s\n", addr.originHost, addr.originService);
    SM_Logs(LOG_INFO, _GTPU_, "Initializing UDP socket for local address binding");
  int status;
  ...
  for(p = servinfo; p != NULL; p = p->ai_next) {
    ...
    if (bind(sockfd, p->ai_addr, p->ai_addrlen) == -1) {
      close(sockfd);
      LOG_W(GTPU,"bind: %s\n", strerror(errno));
            SM_Logs(LOG_INFO, _GTPU_, "Socket binding process sequence failed with the error: \033[0;31m %s \033[0m", strerror(errno));
      continue;
    } else {
  ...
  if (p == NULL) {
    ...
    LOG_E(GTPU,"failed to bind socket: %s %s \n", addr.originHost, addr.originService);
        SM_Logs(LOG_INFO, _GTPU_, "GTP-U establishment process failed with the error: \033[0;31mFAILED TO BIND SOCKET\033[0m");
...
instance_t gtpv1Init(openAddr_t context) {
  ...
  if (id>=0) {
    ...
    // LOG_E(GTPU,"can't create GTP-U instance\n");
        SM_Logs(LOG_INFO, _GTPU_, "Initialising GTP-U Interface Instance: \033[0;31mFailed\033[0m");


  pthread_mutex_unlock(&globGtp.gtp_lock);
  // LOG_I(GTPU, "Created gtpu instance id: %d\n", id);
  SM_Logs(LOG_INFO, _GTPU_,"Successfully instantiated GTP-U interface instance");
