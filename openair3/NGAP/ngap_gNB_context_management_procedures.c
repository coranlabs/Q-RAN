                                     ...
                                     ngap_ue_release_complete_t *ue_release_complete_p)
{
  ...
  if ((ue_context_p = ngap_get_ue_context(ue_release_complete_p->gNB_ue_ngap_id)) == NULL) {
    ...
    SM_Logs(LOG_ERROR, _NGAP_, "Failed to find UE information for UE[ngap] id [%s]",ue_release_complete_p->gNB_ue_ngap_id);
    return -1;
  }
  ...
  if (ngap_gNB_encode_pdu(&pdu, &buffer, &length) < 0) {
    ...
    SM_Logs(LOG_ERROR, _NGAP_, "Failure in encoding  UE Context Release Complete Message");

    return -1;
  }

  SM_Logs(LOG_DEBUG,_NGAP_,"Forwarding UE Context Release Complete Message to SCTP Task.");

  ngap_gNB_itti_send_sctp_data_req(ngap_gNB_instance_p->instance,
                                   ue_context_p->amf_ref->assoc_id, buffer,
                                   length, ue_context_p->tx_stream);
                                   
  SM_Logs(LOG_INFO, _NGAP_, " UE Context Release Complete Message Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
                                ...
                                ngap_ue_release_req_t *ue_release_req_p)
{
  ...
  if ((ue_context_p = ngap_get_ue_context(ue_release_req_p->gNB_ue_ngap_id)) == NULL) {
    ...
    SM_Logs(LOG_ERROR, _NGAP_, "Failed to find UE information for UE[ngap] id [%s] in UE Context Release Req",ue_release_req_p->gNB_ue_ngap_id);
  ...
  if (ngap_gNB_encode_pdu(&pdu, &buffer, &length) < 0) {
    /* Encode procedure has failed... */
    // NGAP_ERROR("Failed to encode UE context release complete\n");
    SM_Logs(LOG_ERROR, _NGAP_, "Failure in encoding  UE Context Release Request Message");
  ...
  SM_Logs(LOG_DEBUG,_NGAP_,"Forwarding UE Context Release Request to SCTP Task.");

  ngap_gNB_itti_send_sctp_data_req(ngap_gNB_instance_p->instance,
                                   ue_context_p->amf_ref->assoc_id, buffer,
                                   length, ue_context_p->tx_stream);

  SM_Logs(LOG_INFO, _NGAP_, "UE Context Release Request Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
