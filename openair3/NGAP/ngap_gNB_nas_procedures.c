...
//------------------------------------------------------------------------------
{
  ...
  SM_Log_Assert(_MAC_, UEfirstReq != NULL, "Received NAS First Request is NULL");
  /* Retrieve the NGAP gNB instance associated with Mod_id */
  instance_p = ngap_gNB_get_instance(instance);
  // DevAssert(instance_p != NULL);
  SM_Log_Assert(_MAC_, instance_p != NULL, "Received NAS First Request is NULL");
  ...
  if (UEfirstReq->ue_identity.presenceMask & NGAP_UE_IDENTITIES_guami) {
    ...
    if (amf_desc_p) {
      ...
      SM_Logs(LOG_INFO, _NGAP_, "Handling NAS First Request.");
      SM_Logs(LOG_INFO, _NGAP_, "AMF Details [%s]: ",amf_desc_p->amf_name);

      printf("| --------------------|--------------|\n");
      printf("| UE ID               | %12d |\n", UEfirstReq->gNB_ue_ngap_id);
      printf("| AMF ID              | %12d |\n", amf_desc_p->assoc_id);
      printf("| Association ID      | %12d |\n", amf_desc_p->assoc_id);
      printf("| GUAMI MCC/MNC       | %6d/%6d |\n", UEfirstReq->ue_identity.guami.mcc,UEfirstReq->ue_identity.guami.mnc);
      printf("| GUAMI AMF Region ID | %12d |\n", UEfirstReq->ue_identity.guami.amf_region_id);
      printf("| GUAMI AMF Set  ID   | %12d |\n", UEfirstReq->ue_identity.guami.amf_set_id);
      printf("| --------------------|--------------|\n");
  ...
  if (amf_desc_p == NULL) {
    ...
    if (UEfirstReq->ue_identity.presenceMask & NGAP_UE_IDENTITIES_FiveG_s_tmsi) {
      ...
      if (amf_desc_p) {
        ...
        printf("| Association ID      | %12d |\n", amf_desc_p->assoc_id);
        printf("| PLMN ID             | %12d |\n", UEfirstReq->selected_plmn_identity);
        printf("| TMSI MCC/MNC        | %6d/%6d |\n",instance_p->plmn[UEfirstReq->selected_plmn_identity].mcc,instance_p->plmn[UEfirstReq->selected_plmn_identity].mnc);
        printf("| TMSI AMF Set ID     | %12d |\n", UEfirstReq->ue_identity.s_tmsi.amf_set_id);
        printf("| --------------------|--------------|\n");
      }
    }
  }
  ...
  if (ngap_gNB_encode_pdu(&pdu, &buffer, &length) < 0) {
      ...
      SM_Logs(LOG_ERROR,_NGAP_,"Error while encoding NAS First Request");
  ...
  SM_Logs(LOG_DEBUG,_NGAP_,"Forwarding NAS First Request to SCTP Task.");
  /* Send encoded message over sctp */
  ngap_gNB_itti_send_sctp_data_req(instance_p->instance, amf_desc_p->assoc_id,
                                   buffer, length, ue_desc_p->tx_stream);

  SM_Logs(LOG_INFO, _NGAP_, "NAS First Request Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
...
//------------------------------------------------------------------------------
{
  ...
  // DevAssert(pdu != NULL);
  SM_Log_Assert(_NGAP_, pdu != NULL, "Received NAS Downlink PDU is NULL.");
  ...
  if ((amf_desc_p = ngap_gNB_get_AMF(NULL, assoc_id, 0)) == NULL) {
    SM_Logs(LOG_ERROR,_NGAP_,"Failed to retrieve existing AMF data for received NAS Downlink Message");
  ...
  SM_Logs(LOG_DEBUG, _NGAP_, "Decoding UE identifiers and NAS message details.");
  ...
  if (0 == ue_desc_p->rx_stream) {
    ...
    SM_Logs(LOG_ERROR, _NGAP_, "Stream mismatch detected for gNB UE NGAP ID: 0x%lx. Expected: %u, Received: %u.",gnb_ue_ngap_id, ue_desc_p->rx_stream, stream);
    return -1;
  }
  ...
  if (ue_desc_p->amf_ue_ngap_id == 0) {
    ...
    if (ue_desc_p->amf_ue_ngap_id != amf_ue_ngap_id) {
      ...
      SM_Logs(LOG_ERROR, _NGAP_, "Mismatch in AMF UE NGAP ID for gNB UE NGAP ID: 0x%lx. Existing: 0x%" PRIx64 ", Received: 0x%" PRIx64 ".",gnb_ue_ngap_id, ue_desc_p->amf_ue_ngap_id, amf_ue_ngap_id);
  ...
  SM_Logs(LOG_INFO, _NGAP_,"NAS Downlink Message Transmission Status for UE ID: 0x%lx - \033[0;32mSuccessful\033[0m.", gnb_ue_ngap_id);       
  return 0;
}
...
//------------------------------------------------------------------------------
{
  ...
  if (ngap_gNB_encode_pdu(&pdu, &buffer, &length) < 0) {
      SM_Logs(LOG_ERROR,_NGAP_,"Error while encoding NAS Uplink Transfer Message");
    // NGAP_ERROR("Failed to encode uplink NAS transport\n");
    /* Encode procedure has failed... */
    return -1;
  }

SM_Logs(LOG_DEBUG,_NGAP_,"Forwarding  NAS Uplink Transfer Message to SCTP Task.");
  ngap_gNB_itti_send_sctp_data_req(ngap_gNB_instance_p->instance,
                                   ue_context_p->amf_ref->assoc_id, buffer,
                                   length, ue_context_p->tx_stream);

  SM_Logs(LOG_INFO, _NGAP_, "NAS Uplink Transfer Message Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
  return 0;
}
...
//------------------------------------------------------------------------------
{
  ...
  if (ngap_gNB_encode_pdu(&pdu, &buffer, &length) < 0) {
    SM_Logs(LOG_ERROR,_NGAP_,"Error while encoding NAS Non-Delivery Indication");
  ...
  SM_Logs(LOG_DEBUG,_NGAP_,"Forwarding  NAS Non-Delivery Indication to SCTP Task.");
  ngap_gNB_itti_send_sctp_data_req(ngap_gNB_instance_p->instance,
                                   ue_context_p->amf_ref->assoc_id, buffer,
                                   length, ue_context_p->tx_stream);

  SM_Logs(LOG_INFO, _NGAP_, "NAS Non-Delivery Indication Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
...
//------------------------------------------------------------------------------
{
  ...
  if (initial_ctxt_resp_p->nb_of_pdusessions){
    ...
    for (i = 0; i < initial_ctxt_resp_p->nb_of_pdusessions; i++) {
      ...
      // NGAP_INFO("initial_ctxt_resp_p: pdusession ID %ld, gnb_addr %d.%d.%d.%d, SIZE %ld, TEID %u\n",
      //            item->pDUSessionID,
  ...
  if (ngap_gNB_encode_pdu(&pdu, &buffer, &length) < 0) {
    SM_Logs(LOG_ERROR,_NGAP_,"Error while encoding Initial Context Setup Response");
    ...
    SM_Logs(LOG_DEBUG,_NGAP_,"Forwarding Initial Context Setup Response to SCTP Task.");
    ...
    SM_Logs(LOG_INFO, _NGAP_, "Initial Context Setup Response Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
...
//------------------------------------------------------------------------------
{
  ...
  if (ngap_gNB_encode_pdu(&pdu, &buffer, &length) < 0) {
    SM_Logs(LOG_ERROR,_NGAP_,"Error while encoding UE Capabilities Message");
    /* Encode procedure has failed... */
  ...
  /* UE associated signalling -> use the allocated stream */
  SM_Logs(LOG_DEBUG,_NGAP_,"Forwarding UE Capabilities Message to SCTP Task.");
  ngap_gNB_itti_send_sctp_data_req(ngap_gNB_instance_p->instance, ue_context_p->amf_ref->assoc_id, buffer, length, ue_context_p->tx_stream);
  SM_Logs(LOG_INFO, _NGAP_, "UE Capabilities Message Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");  
  return 0;
}
...
//------------------------------------------------------------------------------
{
  ...
  if (pdusession_setup_resp_p->nb_of_pdusessions > 0) {
    ...
    for (int i = 0; i < pdusession_setup_resp_p->nb_of_pdusessions; i++) {
      ...
      // NGAP_INFO("pdusession_setup_resp_p: pdusession ID %ld, gnb_addr %d.%d.%d.%d, SIZE %ld, TEID %u\n",
      //            item->pDUSessionID,
      //            tmp->transportLayerAddress.buf[0],
      //            tmp->transportLayerAddress.buf[1],
      //            tmp->transportLayerAddress.buf[2],
  ...
  if (ngap_gNB_encode_pdu(&pdu, &buffer, &length) < 0) {
    SM_Logs(LOG_ERROR,_NGAP_,"Error while encoding PDU Session Setup Response");
  ...
  /* UE associated signalling -> use the allocated stream */
  SM_Logs(LOG_DEBUG,_NGAP_,"Forwarding PDU Session Setup Response to SCTP Task.");
  ngap_gNB_itti_send_sctp_data_req(ngap_gNB_instance_p->instance, ue_context_p->amf_ref->assoc_id, buffer, length, ue_context_p->tx_stream);
  SM_Logs(LOG_INFO, _NGAP_, "PDU Session Setup Response Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");  
  return 0;
}
...
//------------------------------------------------------------------------------
{
  ...
  if (pdusession_modify_resp_p->nb_of_pdusessions > 0) {
    ...
    for (int i = 0; i < pdusession_modify_resp_p->nb_of_pdusessions; i++) {
      ...
      // NGAP_INFO("pdusession_modify_resp: modified pdusession ID %ld\n", item->pDUSessionID);
    }
  }
  ...
  if (ngap_gNB_encode_pdu(&pdu, &buffer, &length) < 0) {
    SM_Logs(LOG_ERROR,_NGAP_,"Error while encoding PDU Session Modify Response");
    // NGAP_ERROR("Failed to encode uplink transport\n");
    /* Encode procedure has failed... */
    return -1;
  }
  ...
  /* UE associated signalling -> use the allocated stream */
  SM_Logs(LOG_DEBUG,_NGAP_,"Forwarding PDU Session Modify Response to SCTP Task.");
  ngap_gNB_itti_send_sctp_data_req(ngap_gNB_instance_p->instance, ue_context_p->amf_ref->assoc_id, buffer, length, ue_context_p->tx_stream);
  SM_Logs(LOG_INFO, _NGAP_, "PDU Session Modify Response Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");  
  return 0;
}
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
{
  ...
  if (ngap_gNB_encode_pdu(&pdu, &buffer, &length) < 0) {
    SM_Logs(LOG_ERROR,_NGAP_,"Error while encoding PDU Session Release Response");
  ...
  /* UE associated signalling -> use the allocated stream */
  SM_Logs(LOG_DEBUG,_NGAP_,"Forwarding PDU Session Release Response to SCTP Task.");
  ngap_gNB_itti_send_sctp_data_req(ngap_gNB_instance_p->instance, ue_context_p->amf_ref->assoc_id, buffer, length, ue_context_p->tx_stream);
  SM_Logs(LOG_INFO, _NGAP_, "PDU Session Release Response Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
