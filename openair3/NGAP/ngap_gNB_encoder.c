...
static inline int ngap_gNB_encode_initiating(NGAP_NGAP_PDU_t *pdu, uint8_t **buffer, uint32_t *len)
{
  ...
  if (i == sizeofArray(tmp)) {
    SM_Logs(LOG_ERROR, _NGAP_, "Received unknown message ID while encoding: [%d]", 
  ...
  if (res.result.encoded <= 0) {
    SM_Logs(LOG_ERROR, _NGAP_, "Failure in encoding Message.");
    return -1;
  }
  else
  {
  SM_Logs(LOG_INFO, _NGAP_, "Successfully encoded Message: [%d]", 
          (int)pdu->choice.initiatingMessage->procedureCode);
  }
...
static inline int ngap_gNB_encode_successfull_outcome(NGAP_NGAP_PDU_t *pdu, uint8_t **buffer, uint32_t *len)
{
  ...
  if (i == sizeofArray(tmp)) {
    ...
    SM_Logs(LOG_ERROR, _NGAP_, "Received unknown message ID while encoding: [%d]",(int)pdu->choice.successfulOutcome->procedureCode);
    return -1;
  }
...
static inline int ngap_gNB_encode_unsuccessfull_outcome(NGAP_NGAP_PDU_t *pdu, uint8_t **buffer, uint32_t *len)
{
  ...
  if (pdu->choice.unsuccessfulOutcome->procedureCode != NGAP_ProcedureCode_id_InitialContextSetup) {
    ...
    SM_Logs(LOG_ERROR, _NGAP_, "Received unknown message ID while encoding: [%d]",(int)pdu->choice.unsuccessfulOutcome->procedureCode);
...
int ngap_gNB_encode_pdu(NGAP_NGAP_PDU_t *pdu, uint8_t **buffer, uint32_t *len)
{
  ...
  switch (pdu->present) {
    ...
    default:
      ...
      SM_Logs(LOG_ERROR, _NGAP_, "Received unknown message while encoding: [%d]",(int)pdu->present);
