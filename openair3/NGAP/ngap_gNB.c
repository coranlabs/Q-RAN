...
static void ngap_gNB_register_amf(ngap_gNB_instance_t *instance_p,
  ...
  SM_Logs(LOG_INFO, _NGAP_, "Initiating validation of AMF registration state");
...
void ngap_gNB_handle_register_gNB(instance_t instance, ngap_register_gnb_req_t *ngap_register_gNB) {
  ...
  if (new_instance != NULL) {
    ...
    SM_Logs(LOG_INFO, _NGAP_, "Initilizaing Detection For New gNB:  \033[0;32mSuccessful\033[0m " );
...
void ngap_gNB_handle_sctp_association_resp(instance_t instance, sctp_new_association_resp_t *sctp_new_association_resp) {
  ...
  SM_Logs(LOG_INFO, _NGAP_, "Building NGAP Setup Request.");
  SM_Logs(LOG_INFO, _NGAP_, "Sending NGAP Setup Request.");
...
void ngap_gNB_init(void) {
  ...
  // NGAP_DEBUG("Starting NGAP layer\n");
  SM_Logs(LOG_INFO, _NGAP_, "Initiating NGAP Layer");
  ...
  if (received_msg) {
    ...
    switch (ITTI_MSG_ID(received_msg)) {
      ...
      case NGAP_REGISTER_GNB_REQ:
        SM_Logs(LOG_INFO, _NGAP_, "NGAP Layer recieved Registration Request");
      ...
      case NGAP_UE_CONTEXT_RELEASE_COMPLETE:
        SM_Logs(LOG_INFO, _NGAP_, "NGAP Layer recieved UE Context Release");
...
//-----------------------------------------------------------------------------
{
  ...
  if (ngap_gNB_encode_pdu(&pdu, &buffer, &len) < 0) {
    SM_Logs(LOG_ERROR,_NGAP_,"Error while encoding NGAP Setup Response");
    // NGAP_ERROR("Failed to encode NG setup request\n");
    return -1;
  }

  /* Non UE-Associated signalling -> stream = 0 */
  SM_Logs(LOG_DEBUG,_NGAP_,"Forwarding NGAP Setup Response to SCTP Task.");
  ngap_gNB_itti_send_sctp_data_req(instance_p->instance, ngap_amf_data_p->assoc_id, buffer, len, 0);
  SM_Logs(LOG_INFO, _NGAP_, "NGAP Setup Response Transmisson Status [from CU]: \033[0;32mSuccessfull\033[0m");
  return ret;
}
