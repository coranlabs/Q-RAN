...
static int ngap_gNB_decode_initiating_message(NGAP_NGAP_PDU_t *pdu) {
  ...
  switch(pdu->choice.initiatingMessage->procedureCode) {
    case NGAP_ProcedureCode_id_DownlinkNASTransport:
      ...
      free(res.buffer);
      SM_Logs(LOG_INFO, _NGAP_, "Decoding Downlink NAS Transport Message.");
      break;

    case NGAP_ProcedureCode_id_InitialContextSetup:
      res = asn_encode_to_new_buffer(NULL, ATS_CANONICAL_XER, &asn_DEF_NGAP_NGAP_PDU, pdu);
      free(res.buffer);
      SM_Logs(LOG_INFO, _NGAP_, "Decoding Initial Context Setup Request.");
      break;

    case NGAP_ProcedureCode_id_UEContextRelease:
      res = asn_encode_to_new_buffer(NULL, ATS_CANONICAL_XER, &asn_DEF_NGAP_NGAP_PDU, pdu);
      free(res.buffer);
      SM_Logs(LOG_INFO, _NGAP_, "Decoding UE Context Release Message.");
      break;

    case NGAP_ProcedureCode_id_Paging:
      res = asn_encode_to_new_buffer(NULL, ATS_CANONICAL_XER, &asn_DEF_NGAP_NGAP_PDU, pdu);
      free(res.buffer);
      SM_Logs(LOG_INFO, _NGAP_, "Decoding Paging Message.");
      break;

    case NGAP_ProcedureCode_id_PDUSessionResourceSetup:
      res = asn_encode_to_new_buffer(NULL, ATS_CANONICAL_XER, &asn_DEF_NGAP_NGAP_PDU, pdu);
      free(res.buffer);
      SM_Logs(LOG_INFO, _NGAP_, "Decoding PDU Session Resource Setup Message.");
      break;

    case NGAP_ProcedureCode_id_PDUSessionResourceModify:
      res = asn_encode_to_new_buffer(NULL, ATS_CANONICAL_XER, &asn_DEF_NGAP_NGAP_PDU, pdu);
      free(res.buffer);
      SM_Logs(LOG_INFO, _NGAP_, "Decoding PDU Session Resource Modify Message.");
      break;

    case NGAP_ProcedureCode_id_PDUSessionResourceRelease:
      res = asn_encode_to_new_buffer(NULL, ATS_CANONICAL_XER, &asn_DEF_NGAP_NGAP_PDU, pdu);
      free(res.buffer);
      SM_Logs(LOG_INFO, _NGAP_, "Decoding PDU Session Resource Release Message.");
      break;

    case NGAP_ProcedureCode_id_ErrorIndication:
      res = asn_encode_to_new_buffer(NULL, ATS_CANONICAL_XER, &asn_DEF_NGAP_NGAP_PDU, pdu);
      free(res.buffer);
      SM_Logs(LOG_INFO, _NGAP_, "Decoding Error Indication Message.");
      break;

    default:
      // NGAP_ERROR("Unknown procedure ID (%d) for initiating message\n",
      //            (int)pdu->choice.initiatingMessage->procedureCode);
      SM_Logs(LOG_ERROR, _NGAP_, "Recieved unknown message id while decoding the message [%d] ",(int)pdu->choice.initiatingMessage->procedureCode);
...
static int ngap_gNB_decode_successful_outcome(NGAP_NGAP_PDU_t *pdu) {
  ...
  switch (pdu->choice.successfulOutcome->procedureCode) {
    case NGAP_ProcedureCode_id_NGSetup:
      res = asn_encode_to_new_buffer(NULL, ATS_CANONICAL_XER, &asn_DEF_NGAP_NGAP_PDU, pdu);
      free(res.buffer);
      SM_Logs(LOG_INFO, _NGAP_, "Decoding NG Setup Message.");
      break;

    case NGAP_ProcedureCode_id_PathSwitchRequest:
      res = asn_encode_to_new_buffer(NULL, ATS_CANONICAL_XER, &asn_DEF_NGAP_NGAP_PDU, pdu);
      free(res.buffer);
      SM_Logs(LOG_INFO, _NGAP_, "Decoding Path Switch Request Message.");
      break;

    case NGAP_ProcedureCode_id_PDUSessionResourceModifyIndication:
      res = asn_encode_to_new_buffer(NULL, ATS_CANONICAL_XER, &asn_DEF_NGAP_NGAP_PDU, pdu);
      free(res.buffer);
      SM_Logs(LOG_INFO, _NGAP_, "Decoding PDU Session Resource Modify Indication Message.");
      break;

    default:
      // NGAP_ERROR("Unknown procedure ID (%d) for successful outcome message\n",
      //            (int)pdu->choice.successfulOutcome->procedureCode);
      SM_Logs(LOG_ERROR, _NGAP_, "Recieved unknown message id while decoding the message [%d] ",(int)pdu->choice.successfulOutcome->procedureCode);
      return -1;
  }
...
static int ngap_gNB_decode_unsuccessful_outcome(NGAP_NGAP_PDU_t *pdu) {
  ...
  switch (pdu->choice.unsuccessfulOutcome->procedureCode) {
    ...
    default:
      // NGAP_ERROR("Unknown procedure ID (%d) for unsuccessful outcome message\n",
      //            (int)pdu->choice.unsuccessfulOutcome->procedureCode);
      SM_Logs(LOG_ERROR, _NGAP_, "Recieved unknown message id while decoding the message [%d] ",(int)pdu->choice.unsuccessfulOutcome->procedureCode);
      return -1;
  }
...
int ngap_gNB_decode_pdu(NGAP_NGAP_PDU_t *pdu, const uint8_t *const buffer,
  ...
  if (dec_ret.code != RC_OK) {
    ...
    SM_Logs(LOG_ERROR, _NGAP_, "Failure in encoding ASN1 Message");
