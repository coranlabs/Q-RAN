...
void ngap_handle_ng_setup_message(ngap_gNB_amf_data_t *amf_desc_p, int sctp_shutdown) {
  if (sctp_shutdown) {
    ...
    SM_Logs(LOG_INFO, _NGAP_, "NGAP Setup Response Message received.");
...
static int ngap_gNB_handle_ng_setup_failure(sctp_assoc_t assoc_id, uint32_t stream, NGAP_NGAP_PDU_t *pdu)
{
  ...
  if (stream != 0) {
    SM_Logs(LOG_ERROR, _NGAP_, "NGAP Setup Response [Failure] Message received.");
  ...
  if ((ie->value.choice.Cause.present == NGAP_Cause_PR_misc) &&
    ...
    SM_Logs(LOG_ERROR, _NGAP_, "NGAP Setup Response reports failure.");
...
static int ngap_gNB_handle_ng_setup_response(sctp_assoc_t assoc_id, uint32_t stream, NGAP_NGAP_PDU_t *pdu)
{
  ...
  if ((amf_desc_p = ngap_gNB_get_AMF(NULL, assoc_id, 0)) == NULL) {
    SM_Logs(LOG_ERROR, _NGAP_, "NGAP Setup Response Message: AMF Info mismatch !");
...
{
  SM_Logs(LOG_INFO, _NGAP_, " Received PDU Session Release command.");
  int i;
  ngap_gNB_amf_data_t *amf_desc_p = NULL;
  ...
  SM_Logs(LOG_INFO, _NGAP_, "PDU Session Release command Aria UE ID [%lu] AMF NGAP ID [%lu]",assoc_id, gnb_ue_ngap_id, amf_ue_ngap_id);
...
int ngap_gNB_handle_message(sctp_assoc_t assoc_id, int32_t stream, const uint8_t *const data, const uint32_t data_length)
{
  ...
  if (ngap_gNB_decode_pdu(&pdu, data, data_length) < 0) {
    ...
    SM_Logs(LOG_ERROR, _NGAP_, "Failed to Decode PDU while handling NGAP message.");
    return -1;
  }
  else{
    SM_Logs(LOG_INFO, _NGAP_, "Handling NGAP Message, PDU decoding success");
  }
