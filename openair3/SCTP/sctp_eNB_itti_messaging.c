                                   ...
                                   uint16_t stream)
{
  ...
  if (message_p == NULL) {
      SM_Logs(LOG_ERROR, _SCTP_, "Failed to allocate new message for SCTP Message.");
      return -1;
  }
  ...
  if (sctp_data_ind_p->buffer == NULL) {
      SM_Logs(LOG_ERROR, _SCTP_, "Failed to allocate memory for SCTP message buffer.");
      return -1;
  }
