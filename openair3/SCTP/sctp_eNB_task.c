...
#include "../../pqc/dtls/dtls.h"

/* Used to format an uint32_t containing an ipv4 address */
#define IPV4_ADDR "%u.%u.%u.%u"
#define IPV4_ADDR_FORMAT(aDDRESS)                                                                                         \
  (uint8_t)((aDDRESS) & 0x000000ff), (uint8_t)(((aDDRESS) & 0x0000ff00) >> 8), (uint8_t)(((aDDRESS) & 0x00ff0000) >> 16), \
      (uint8_t)(((aDDRESS) & 0xff000000) >> 24)

enum sctp_connection_type_e { SCTP_TYPE_CLIENT, SCTP_TYPE_SERVER, SCTP_TYPE_MULTI_SERVER, SCTP_TYPE_MAX };

/* SSL Relevant variables */
static uint8_t handshake_done = 0;
static OSSL_LIB_CTX *libctx = NULL;
static const char *modulename = "oqsprovider";
static char *configfile = "../../../pqc/oqs-provider/test/oqs.cnf";
static OSSL_PROVIDER *provider = NULL;

/* Setup global SSL_CTX */
SSL_CTX *server_ctx;
SSL_CTX *client_ctx;


void XorBuffer(unsigned char *buffer, size_t len, unsigned char key)
{
...
int init_global_server_ctx()
{
  if (provider == NULL) {
    int ret2 = init_global_ossl_provider();
    if (ret2 != 0)
      return -1;
  }
  SM_Logs(LOG_INFO, _DTLS_, "Server CTX\n");

  if (server_ctx == NULL) {
    const char *cert_file = "../../../openair3/SCTP/server.crt";
    const char *key_file = "../../../openair3/SCTP/server.key";

    server_ctx = dtls_server_ctx_init(provider, libctx, cert_file, key_file);
    if (server_ctx == NULL) {
      cleanup_ssl_ctx(server_ctx);
      LOG_E(SCTP, "error in ctx");
      return -1;
    }
    fprintf(stdout, "Certificate loaded: MLDSA65_P384 (Dilithium3 + P384 Hybrid)\n");
...
int init_global_client_ctx()
{
  ...
  if (client_ctx == NULL) {
    const char *cert_file = "../../../openair3/SCTP/client.crt";
    const char *key_file = "../../../openair3/SCTP/client.key";

    client_ctx = dtls_client_ctx_init(provider, libctx, cert_file, key_file);
    if (client_ctx == NULL) {
      cleanup_ssl_ctx(client_ctx);
      LOG_E(SCTP, "error in ctx");
      return -1;
    }
    SM_Logs(LOG_INFO, _DTLS_, "Client DTLS Context created.\n");
...
int init_global_ossl_provider()
{
  int ret = init_global_libctx();
  if (ret != 0) {
    return -1;
  }

  if (provider == NULL) {
    provider = load_oqs_provider(libctx, modulename, configfile);
    if (provider == NULL) {
      fprintf(stderr, "OQS Provider could not be loaded.\n");
      return -1;
    } else {
      SM_Logs(LOG_INFO, _DTLS_, "OSSL provider - oqs loaded.\n");
    }
  }
...
int init_global_libctx()
{
  if (libctx == NULL) {
    libctx = load_ossl_libctx();
    if (libctx == NULL) {
      fprintf(stderr, "Libctx could not be loaded.\n");
      return -1;
    }
    SM_Logs(LOG_INFO, _DTLS_, "Library context loaded.\n");
  }
...
static inline void sctp_eNB_accept_associations_multi(struct sctp_cnx_list_elm_s *sctp_cnx)
{
  ...
  n = sctp_recvmsg(sctp_cnx->sd, (void *)buffer, SCTP_RECV_BUFFER_SIZE, (struct sockaddr *)&addr, &from_len, &sinfo, &flags);

  if (n < 0) {
    if (errno == ENOTCONN) {
      // SCTP_DEBUG("Received not connected for sd %d\n", sctp_cnx->sd);
      SM_Logs(LOG_ERROR, _SCTP_, "Invalid socket descriptor ");
      close(sctp_cnx->sd);
    } else {
      // SCTP_DEBUG("An error occured during read\n");
      // SCTP_ERROR("sctp_recvmsg (fd %d, len %d ): %s:%d\n", sctp_cnx->sd, n, strerror(errno), errno);
      SM_Logs(LOG_ERROR, _SCTP_, "sctp_recvmsg : Error while reading ");
    }
    return;
  }
    ...
    if (SCTP_ASSOC_CHANGE == snp->sn_header.sn_type) {
      ...
      switch (sctp_assoc_changed->sac_state) {
        case SCTP_COMM_UP: {
          ...
          if (sctp_get_sockinfo(ns, &new_cnx->in_streams, &new_cnx->out_streams, &new_cnx->assoc_id) != 0) {
            // SCTP_ERROR("sctp_get_sockinfo failed\n");
            SM_Logs(LOG_ERROR, _SCTP_, "sctp_get_sockinfo failed \n");
            close(ns);
            free(new_cnx);
            return;
          }
          ...
          sctp_itti_send_association_ind(new_cnx->task_id,
                                         new_cnx->instance,
                                         new_cnx->assoc_id,
                                         new_cnx->local_port,
                                         new_cnx->out_streams,
...
//------------------------------------------------------------------------------
static void sctp_handle_new_association_req_multi(const instance_t instance,
                                                  ...
                                                  const sctp_new_association_req_multi_t *const sctp_new_association_req_p)
{
   ...
   */
  if ((sctp_new_association_req_p->remote_address.ipv6 != 0) || (sctp_new_association_req_p->remote_address.ipv4 != 0)) {
    uint8_t address_index = 0;
    uint8_t used_address = sctp_new_association_req_p->remote_address.ipv6 + sctp_new_association_req_p->remote_address.ipv4;
    ...
    if (sctp_new_association_req_p->remote_address.ipv6 == 1) {
      if (inet_pton(AF_INET6, sctp_new_association_req_p->remote_address.ipv6_address, &addr[address_index].sin_addr.s_addr) != 1) {
        ...
        SM_Logs(LOG_ERROR,
                _SCTP_,
                "Unable to convert IPv6 address for: [%c]",
                sctp_new_association_req_p->remote_address.ipv6_address);
        // close(sd);
        // return;
    ...
    if (sctp_new_association_req_p->remote_address.ipv4 == 1) {
      if (inet_pton(AF_INET, sctp_new_association_req_p->remote_address.ipv4_address, &addr[address_index].sin_addr.s_addr) != 1) {
        ...
        SM_Logs(LOG_ERROR,
                _SCTP_,
                "Unable to convert IPv4 address for: [%c]",
                sctp_new_association_req_p->remote_address.ipv6_address);
    ...
    if (sctp_connectx(sd, (struct sockaddr *)addr, 1, &assoc_id) < 0) {
      ...
      if (errno != EINPROGRESS) {
        ...
        SM_Logs(LOG_ERROR, _SCTP_, "sctp_connectx failed \n");
  ...
  if (ns == -1) {
    ...
    SM_Logs(LOG_ERROR, _SCTP_, "sctp_peeloff: failed \n", sctp_new_association_req_p->remote_address.ipv6_address);
  }
                                            ...
                                            const sctp_new_association_req_t *const req)
{
  ...
  if (requestor == TASK_DU_F1 || requestor == TASK_CU_F1) {
    ssl_needed = 1;

    if (connection_type == SCTP_TYPE_CLIENT) {
      if (init_global_client_ctx() != 0) {
        fprintf(stderr, "Error in setting up DTLS CLIENT CTX\n");
      }

    } else {
      if (init_global_server_ctx() != 0) {
        fprintf(stderr, "Error in setting up DTLS Server CTX\n");
      }
    }
  }
  ...
  //   SM_Logs(LOG_ERROR, _SCTP_, "getaddrinfo: failed \n");
  ...
  for (p = serv; p != NULL; p = p->ai_next) {
    ...
    if ((sd = socket(serv->ai_family, serv->ai_socktype, serv->ai_protocol)) == -1) {
      ...
      SM_Logs(LOG_ERROR, _SCTP_, "Socket creation failed \n");
      continue;
    }
    ...
    // SM_Logs(LOG_ERROR, _SCTP_, "sctp_set_init_opt: failed \n");
    ...
    // SM_Logs(LOG_ERROR, _SCTP_, "setsockopt: failed \n");
    ...
    if (ret != 0) {
      ...
      SM_Logs(LOG_ERROR, _SCTP_, "Socket binding failed");
      close(sd);
      continue;
    }
  ...
  if (p == NULL) {
    ...
    SM_Logs(LOG_ERROR, _SCTP_, "Unable to open socket");
    return;
  }

  /* add ssl if required */
  int ret;
  Client *client = (Client *)malloc(sizeof(struct ssl_client));
  if (client == NULL) {
  ...
  if (ssl_needed == 1) {
    if (connection_type == SCTP_TYPE_CLIENT) {
      ret = ssl_client_init(client_ctx, client, sd, 8192, SSLMODE_CLIENT);
      if (ret == -1) {
        fprintf(stderr, "DTLS OBJ (Client) NOT SETUP.\n");
        close(sd);
        return;
      }
      SM_Logs(LOG_INFO, _DTLS_, "DTLS Client bound to the socket.\n");
    } else {
      ret = ssl_client_init(server_ctx, client, sd, 8192, SSLMODE_SERVER);
      if (ret == -1) {
        fprintf(stderr, "DTLS OBJ (Server) NOT SETUP.\n");
        close(sd);
        return;
      }
    }
  ...
  if (remote != NULL) {
    ...
    for (p = serv; p != NULL; p = p->ai_next) {
      ...
      if (sctp_connectx(sd, p->ai_addr, 1, &assoc_id) < 0) {
        ...
        if (errno != EINPROGRESS) {
          //   SCTP_ERROR("Connect failed: %s\n", strerror(errno));
          SM_Logs(LOG_ERROR, _SCTP_, "Connection failed\n");
...
static int sctp_close_association(sctp_close_association_t *close_association_p)
{
  ...
  if (sctp_cnx == NULL) {
    ...
    SM_Logs(LOG_ERROR, _SCTP_, "Unable to find sctpd for assoc_id : [%d] \n", close_association_p->assoc_id);
...
static int sctp_create_new_listener(const instance_t instance, const task_id_t requestor, sctp_init_t *init_p, int server_type)
{
  ...
  if (init_p == NULL) {
    SM_Logs(LOG_CRTERR, _ARIA_, "Null pointer encountered  ");
    return;
  }

  // Ensure the bind address is not NULL
  // DevAssert(init_p->bind_address != NULL);
  if (init_p->bind_address == NULL) {
    SM_Logs(LOG_ERROR, _ARIA_, "Null pointer encountered for bind address");
    return -1;
  }
  ...
  SM_Logs(LOG_DEBUG, _SCTP_, "Configurations: bind_address=%s, port=%d", local, init_p->port);
  ...
  // AssertFatal(status == 0, "getaddrinfo() failed: %s\n", gai_strerror(status));
  SM_Logs(LOG_DEBUG, _SCTP_, "Address resolution for bind_address=%s, port=%s completed", local, port);
  ...
  for (p = serv; p != NULL; p = p->ai_next) {
    ...
    if ((sd = socket(serv->ai_family, socktype, serv->ai_protocol)) == -1) {
      // SCTP_WARN("Socket creation failed: %s\n", strerror(errno));
      SM_Logs(LOG_WARN, _SCTP_, "Error while creating socket for IP=%s: %s", ip, strerror(errno));
      continue;
    }
    ...
    // AssertFatal(ret == 0, "sctp_set_init_opt() failed\n");
    // SM_Logs(LOG_ERROR, _SCTP_, "sctp_set_init_opt: failed \n");
    SM_Logs(LOG_DEBUG, _SCTP_, "SCTP initialization options set: In Streams=%d, Out Streams=%d", in_streams, out_streams);
    ...
    ret = setsockopt(sd, serv->ai_protocol, SCTP_EVENTS, &event, sizeof(event));
    // AssertFatal(ret == 0, "setsockopt() IPPROTO_SCTP_EVENTS failed: %s\n", strerror(errno));
    // SM_Logs(LOG_ERROR, _SCTP_, "setsockopt: failed \n");
    if (ret == 0) {
      SM_Logs(LOG_INFO, _SCTP_, "setsockopt returns success, Socket Descriptor : [%d]", sd);
    } else {
      SM_Logs(LOG_CRTERR, _SCTP_, "setsockopt returns failure, Socket Descriptor : [%d]", sd);
    }
    ...
    if (ret != 0) {
      // SCTP_WARN("sctp_bindx() SCTP_BINDX_ADD_ADDR failed: errno %d %s\n", errno, strerror(errno));
      SM_Logs(LOG_WARN, _SCTP_, "Socket binding failed for IP=%s: %s", ip, strerror(errno));
      close(sd);
      continue;
    }
    ...
    if (listen(sd, 5) < 0) {
      ...
      SM_Logs(LOG_WARN, _SCTP_, "[listen()] function returned failure for IP=%s: %s", ip, strerror(errno));
      close(sd);
      return -1;
    }

    // SCTP_DEBUG("Created listen socket %d on %s:%s local %s\n", sd, ip, port, local);
    SM_Logs(LOG_INFO, _SCTP_, "Successfully created and bound listening socket on IP=%s, port=%s", ip, port);
    break;
  }
  ...
  if (p == NULL) {
    ...
    SM_Logs(LOG_ERROR, _SCTP_, "Failed to open server socket: no SCTP listener active");
    return -1;
  }

  // Allocate memory for SCTP connection structure
  struct sctp_cnx_list_elm_s *sctp_cnx = calloc(1, sizeof(*sctp_cnx));
  if (sctp_cnx != NULL) {
    SM_Logs(LOG_DEBUG, _SCTP_, "Allocated memory for SCTP connection structure");
  } else {
    SM_Logs(LOG_CRTERR, _SCTP_, "Allocated memory for SCTP connection structure Failed !");
  }

  // AssertFatal(sctp_cnx != NULL, "out of memory\n");
  // SM_Logs(LOG_ERROR, _SCTP_, "out of memory \n");

  int ctx_init_retval = init_global_server_ctx();
  if (ctx_init_retval == -1) {
    fprintf(stdout, "SSL CTX creation failed\n");
    return -1;
  } else {
    fprintf(stdout, "SSL CTX created successfully.\n");
  }
  ...
  STAILQ_INSERT_TAIL(&sctp_cnx_list, sctp_cnx, entries);
  sctp_nb_cnx++;
  SM_Logs(LOG_DEBUG, _SCTP_, "Inserted new SCTP connection into the list. Total connections: %d", sctp_nb_cnx);

  // Add the socket to the list of FDs monitored by ITTI
  itti_subscribe_event_fd(TASK_SCTP, sd);
...
//------------------------------------------------------------------------------
static inline void sctp_eNB_accept_associations(struct sctp_cnx_list_elm_s *sctp_cnx)
{
  int client_sd;
  ...
  // // DevAssert(sctp_cnx != NULL);
  if (sctp_cnx == NULL) {
    SM_Logs(LOG_ERROR, _ARIA_, "Connection pointer is NULL. Cannot accept new association.");
    return;
  }
  ...
  if ((client_sd = accept(sctp_cnx->sd, (struct sockaddr *)&saddr, &saddr_size)) < 0) {
    ...
    SM_Logs(LOG_ERROR, _SCTP_, "Failed to accept connection on socket [%d]: %s", sctp_cnx->sd, strerror(errno));
    return;
  }

  SM_Logs(LOG_INFO, _SCTP_, "Accepted new client connection on socket [%d]", client_sd);

  struct sctp_cnx_list_elm_s *new_cnx;
  uint16_t port;

  /* This is an ipv6 socket */
  port = ntohs(saddr.sin6_port); // Convert to host byte order

  /* Contrary to BSD, client socket does not inherit O_NONBLOCK option */
  if (fcntl(client_sd, F_SETFL, O_NONBLOCK) < 0) {
    // SCTP_ERROR("fcntl F_SETFL O_NONBLOCK failed: %s\n", strerror(errno));
    SM_Logs(LOG_ERROR, _SCTP_, "Failed to configure non-blocking mode for client socket [%d]: %s", client_sd, strerror(errno));
    close(client_sd);
    return;
  }

  /* now handle SSL_accept: */

  Client *client = (Client *)malloc(sizeof(struct ssl_client));
  if (sctp_cnx->task_id == TASK_DU_F1 || sctp_cnx->task_id == TASK_CU_F1 || sctp_cnx->task_id == TASK_CUCP_E1
      || sctp_cnx->task_id == TASK_CUUP_E1 || sctp_cnx->task_id == TASK_NGAP) {
    if (init_global_server_ctx() != 0) {
      close(client_sd);
      return;
    }
    ...
    int ret = ssl_client_init(server_ctx, client, client_sd, 8192, SSLMODE_SERVER);
    if (ret == -1) {
      SCTP_ERROR("error in creating dtls client\n");
      close(client_sd);
      return;
    } else {
      SM_Logs(LOG_INFO, _DTLS_, "SSL Client created");
    }

    SM_Logs(LOG_DEBUG, _SCTP_, "Configured non-blocking mode for client socket [%d]", client_sd);

    new_cnx = calloc(1, sizeof(*sctp_cnx));
    // DevAssert(new_cnx != NULL);
    if (new_cnx == NULL) {
      SM_Logs(LOG_ERROR, _ARIA_, "Failed to allocate memory for new client connection.");
      close(client_sd);
      return;
    }

    SM_Logs(LOG_DEBUG, _SCTP_, "Allocated memory for new client connection.");
    ...
    new_cnx->instance = sctp_cnx->instance;
    new_cnx->local_port = sctp_cnx->local_port;
    new_cnx->ssl_client = client;

    if (sctp_get_sockinfo(client_sd, &new_cnx->in_streams, &new_cnx->out_streams, &new_cnx->assoc_id) != 0) {
      // SCTP_ERROR("sctp_get_sockinfo failed\n");
      SM_Logs(LOG_ERROR, _SCTP_, "Failed to retrieve socket information for client socket [%d]", client_sd);
      close(client_sd);
      free(new_cnx);
      return;
    }
    ...
    SM_Logs(LOG_INFO,
            _SCTP_,
            "Socket information retrieved: assoc_id=%d, In Streams=%d, Out Streams=%d",
            new_cnx->assoc_id,
            new_cnx->in_streams,
            new_cnx->out_streams);
    ...
    SM_Logs(LOG_INFO, _SCTP_, "New client connection added to the list. Total connections: %d", sctp_nb_cnx);
    ...
    sctp_itti_send_association_ind(new_cnx->task_id,
                                   ...
                                   new_cnx->out_streams,
                                   new_cnx->in_streams);
    SM_Logs(LOG_INFO, _SCTP_, "Association indication sent for client socket [%d], port=%d", client_sd, port);
  }
}
//------------------------------------------------------------------------------
static inline void sctp_eNB_read_from_socket(struct sctp_cnx_list_elm_s *sctp_cnx)
{
  // Ensure the connection object is valid
  if (sctp_cnx == NULL) {
    SM_Logs(LOG_ERROR, _SCTP_, "Connection object is invalid. Cannot proceed with reading.");
    return;
  }
  ...
  uint8_t buffer[SCTP_RECV_BUFFER_SIZE];
  uint8_t buffer_copy[SCTP_RECV_BUFFER_SIZE];

  // Attempt to read from the socket
  int n = 0;
  n = sctp_recvmsg(sctp_cnx->sd, (void *)buffer, SCTP_RECV_BUFFER_SIZE, NULL, NULL, &sinfo, &flags);
  ...
  if (n < 0) {
    if ((errno == ENOTCONN) || (errno == ECONNRESET) || (errno == ETIMEDOUT) || (errno == ECONNREFUSED)) {
      ...
      SM_Logs(LOG_ERROR, _SCTP_, "Socket is no longer connected. Cleaning up resources.");
      sctp_itti_send_association_resp(sctp_cnx->task_id,
                                      sctp_cnx->instance,
                                      sctp_cnx->assoc_id,
                                      sctp_cnx->cnx_id,
                                      SCTP_STATE_UNREACHABLE,
      ...
      SM_Logs(LOG_ERROR, _SCTP_, "An error occurred while reading from the socket. System error: %s", strerror(errno));
    }
    return;
  } else if (n == 0) {
    SM_Logs(LOG_INFO, _SCTP_, "Socket read returned zero. No data available.");
    return;
  }

  if (!(flags & MSG_EOR)) {
    SM_Logs(LOG_ERROR, _SCTP_, "Partial messages are not supported. Aborting operation.");
    return;
  }
  ...
  if (flags & MSG_NOTIFICATION) {
    ...
    if (SCTP_SHUTDOWN_EVENT == snp->sn_header.sn_type) {
      ...
      SM_Logs(LOG_ERROR, _SCTP_, "Received shutdown notification. Closing connection.");
      close(sctp_cnx->sd);
      ...
      struct sctp_assoc_change *sctp_assoc_changed = &snp->sn_assoc_change;

      SM_Logs(LOG_DEBUG, _SCTP_, "Received association change notification. Processing state change.");

      switch (sctp_assoc_changed->sac_state) {
        case SCTP_COMM_UP:
          if (sctp_get_peeraddresses(sctp_cnx->sd, NULL, NULL) != 0) {
            SM_Logs(LOG_ERROR, _SCTP_, "Failed to retrieve peer addresses during association change.");
          } else {
            sctp_get_sockinfo(sctp_cnx->sd, &sctp_cnx->in_streams, &sctp_cnx->out_streams, &sctp_cnx->assoc_id);
          }
          SM_Logs(LOG_INFO,
                  _SCTP_,
                  "New association established: In Streams=%d, Out Streams=%d",
                  sctp_cnx->in_streams,
        ...
        case SCTP_SHUTDOWN_COMP:
          SM_Logs(LOG_ERROR, _SCTP_, "Connection shutdown completed.");
          break;

        case SCTP_RESTART:
          SM_Logs(LOG_ERROR, _SCTP_, "Connection restarted by peer.");
          break;

        case SCTP_CANT_STR_ASSOC:
          SM_Logs(LOG_ERROR, _SCTP_, "Unable to start a new association. Resource constraints.");
          break;

        case SCTP_COMM_LOST:
          SM_Logs(LOG_ERROR, _SCTP_, "Communication with peer lost.");
          break;

        default:
          SM_Logs(LOG_WARN, _SCTP_, "Unhandled association state change. State ID=%d", sctp_assoc_changed->sac_state);
          break;
      }
    }
  } else {
    ...
    if (ntohl(sinfo.sinfo_ppid) != sctp_cnx->ppid) { // for now
      // SM_Logs(LOG_ERROR, _SCTP_, "Unexpected message received with a mismatched identifier.");
    }

    if (n != 256)
      SM_Logs(LOG_DEBUG, _SCTP_, "Message received: Length=%d, Stream=%d", n, sinfo.sinfo_stream);
    ...
    if (sctp_cnx->ssl_client != NULL && sctp_cnx->ssl_client->handshake_done == 1) {
      ...
      sctp_itti_send_new_message_ind(sctp_cnx->task_id, sctp_cnx->instance, sinfo.sinfo_assoc_id, buffer, n, sinfo.sinfo_stream);
    }
  }
}

static enum sslstatus sctp_ssl_do_handshake(sctp_ssl_do_handshake_t *sctp_ssl_do_handshake_p)
{
  struct sctp_cnx_list_elm_s *sctp_cnx = NULL;
  sctp_cnx = sctp_get_cnx(sctp_ssl_do_handshake_p->assoc_id, 0); // if assoc_id is defined, it will compare that only.

  // fprintf(stdout, "TASK doing SSL handshake: %d\n", sctp_cnx->task_id);

  Client *c = sctp_cnx->ssl_client;
  int status = do_ssl_handshake(c);

  // fprintf(stdout, "Handshake status: %d\n", status);

  // Send sctp notification that the SSL handshake is done and we can proceed F1 Setup REQ msg.
  if (status == SSLSTATUS_OK) {
    fprintf(stdout, "SSL handshake done.\n");
    for (int i = 0; i < 20; i++) { // first 20 ciphers
      const char *cipher = SSL_get_cipher_list(sctp_cnx->ssl_client->ssl, i);
      if (cipher == NULL)
        break; // less ciphers than the priority available.
      SM_Logs(LOG_INFO, _DTLS_, "SSL cipher available: %d:%s", i + 1, cipher);
    }

    const char *cipher = SSL_get_cipher(sctp_cnx->ssl_client->ssl);
    if (cipher) {
      SM_Logs(LOG_INFO, _DTLS_, "SSL cipher negotiated: %s", cipher);
    }

    long group_name_nid = SSL_get_negotiated_group(sctp_cnx->ssl_client->ssl);
    if (group_name_nid != 0) {
      // Convert NID to a human-readable string
      const char *group_name = OBJ_nid2ln(group_name_nid);
      if (group_name != NULL) {
        SM_Logs(LOG_INFO, _DTLS_, "Negotiated group: %s", group_name);
      } else {
        SM_Logs(LOG_WARN, _DTLS_,"Group NID found, but name is unknown.");
      }
    } else {
      printf("No group negotiated or error occurred.");
    }
    c->handshake_done = 1;
    return SSLSTATUS_OK;
  }
...
//------------------------------------------------------------------------------
void static sctp_eNB_flush_sockets(struct epoll_event *events, int nb_events)
{
  int i;
  struct sctp_cnx_list_elm_s *sctp_cnx = NULL;
...
static void sctp_eNB_process_itti_msg()
{
  ...
  if (received_msg != NULL) {
    ...
    switch (ITTI_MSG_ID(received_msg)) {
      case SCTP_INIT_MSG: {
        fprintf(stdout, "SCTP_INIT_MSG\n");
        if (sctp_create_new_listener(ITTI_MSG_DESTINATION_INSTANCE(received_msg),
                                     ITTI_MSG_ORIGIN_ID(received_msg),
                                     &received_msg->ittiMsg.sctp_init,
                                     0)
            < 0) {

          SM_Logs(LOG_ERROR, _SCTP_, "listenere not created ");
        }
      } break;

      case SCTP_INIT_MSG_MULTI_REQ: {
        int multi_sd = sctp_create_new_listener(ITTI_MSG_DESTINATION_INSTANCE(received_msg),
                                                ITTI_MSG_ORIGIN_ID(received_msg),
                                                &received_msg->ittiMsg.sctp_init_multi,
                                                1);
        ...
        sctp_itti_send_init_msg_multi_cnf(ITTI_MSG_ORIGIN_ID(received_msg), ITTI_MSG_DESTINATION_INSTANCE(received_msg), multi_sd);
      } break;
      ...
      case SCTP_SSL_DO_HANDSHAKE: {
        // SM_Logs(LOG_INFO,_DTLS_,"Received new handshake request - sent by task: %d", ITTI_MSG_ORIGIN_ID(received_msg));
        SM_Logs(LOG_INFO, _DTLS_, "Received new DTLS handshake request");
        int status = SSLSTATUS_FAIL;
        while (status != SSLSTATUS_OK) {
          status = sctp_ssl_do_handshake(&received_msg->ittiMsg.sctp_ssl_do_handshake);
          handshake_done = 1;
        }
        break;
      }
      ...
      default:
        ...
        //    ITTI_MSG_ID(received_msg), ITTI_MSG_NAME(received_msg));
        SM_Logs(LOG_ERROR, _SCTP_, "Invalid message received");
        break;
    }
