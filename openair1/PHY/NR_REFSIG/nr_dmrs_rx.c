                     ...
                     uint8_t config_type)
{
  ...
  if ((p >= 1000) && (p < ((config_type == NFAPI_NR_DMRS_TYPE1) ? 1008 : 1012))) {
    ...
    SM_Logs(LOG_ERROR,_PHY_,"Incorrect Pdsch dmrs port");
  }
                                  ...
                                  uint8_t dmrs_type)
{
  ...
  if (dmrs_type != pusch_dmrs_type1){
    SM_Logs(LOG_ERROR,_PHY_,"Incorrect Pusch dmrs conf type");
  }
    // LOG_E(PHY,"PUSCH DMRS config type %d not valid\n", dmrs_type);
  ...
  if ((p>=1000) && (p<1008)) {
    ...
    // LOG_E(PHY,"Illegal p %d PUSCH DMRS port\n",p);
    SM_Logs(LOG_ERROR,_PHY_,"Incorrect Pusch dmrs port");
  }
