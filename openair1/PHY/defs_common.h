...
static inline void wait_sync(char *thread_name)
{
  ...
  SM_Log_Assert(_ARIA_, (rc = pthread_mutex_lock( &sync_mutex ))==0,"sync_mutex lock error");
  // AssertFatal((rc = pthread_mutex_lock( &sync_mutex ))==0,"sync mutex lock error");

  while (sync_var<0 && !oai_exit)
    pthread_cond_wait( &sync_cond, &sync_mutex );

  SM_Log_Assert(_ARIA_, (rc = pthread_mutex_unlock( &sync_mutex ))==0,"sync_mutex unlock error");
...
static inline int wakeup_thread(pthread_mutex_t *mutex,
  ...
  SM_Log_Assert(_ARIA_, (rc = pthread_mutex_lock(mutex))==0,"mutex lock error");
  ...
  while (*instance_cnt == 0 && !oai_exit) {
    ...
    SM_Log_Assert(_ARIA_, (rc = pthread_mutex_unlock(mutex))==0,"mutex unlock error");
