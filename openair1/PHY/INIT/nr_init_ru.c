...
int nr_phy_init_RU(RU_t *ru)
{
  ...
  // LOG_D(PHY, "Initializing RU signal buffers (if_south %s) nb_tx %d, nb_rx %d\n", ru_if_types[ru->if_south], ru->nb_tx, ru->nb_rx);
    SM_Logs(LOG_INFO, _PHY_, "RU Signal Buffer Initialization: Interface: %s (Southbound) | TX Buffers: %d | RX Buffers: %d | Remote Radio Unit (RRU) Configuration in Progress.\n",ru_if_types[ru->if_south],ru->nb_tx, ru->nb_rx);
