...
void nr_feptx_prec(RU_t *ru, int frame_tx, int tti_tx)
{
  ...
  if (ru->nb_log_antennas <= 0) {
    SM_Logs(LOG_ERROR, _F1AP_, "ru->nb_log_antennas is 0 or negative: %d\n", ru->nb_log_antennas);
    ...
    if (ru->config.dbt_config.num_dig_beams == 0 || gNB->common_vars.analog_bf) {
      ...
      for (int l = 0; l < fp->symbols_per_slot; l++) {
        for (int aa = 0; aa < ru->nb_tx; aa++) {
          SM_Logs(LOG_ERROR, _F1AP_, "This needs to be fixed by using appropriate beams from config\n");
