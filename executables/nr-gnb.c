...
void init_gNB_Tpool(int inst) {
  ...
  // ULSCH decoding threadpool
  initTpool(global_configs.thread_pool_cores, &gNB->threadPool, cpumeas(CPUMEAS_GETSTATE));
  ...
  // create the RX thread responsible for RX processing start event
  threadCreate(&gNB->L1_rx_thread, L1_rx_thread, (void *)gNB, "L1_rx_thread", gNB->L1_rx_thread_core, OAI_PRIORITY_RT_MAX);

  // create the TX thread responsible for TX processing start event
  threadCreate(&gNB->L1_tx_thread, L1_tx_thread, (void *)gNB, "L1_tx_thread", gNB->L1_tx_thread_core, OAI_PRIORITY_RT_MAX);

  // Allocate memory for L1 TX message
  notifiedFIFO_elt_t *msgL1Tx = newNotifiedFIFO_elt(sizeof(processingData_L1tx_t), 0, &gNB->L1_tx_out, NULL);
  ...
  // Conditional thread creation for L1 stats
  // if ((!get_softmodem_params()->emulate_l1) && (!IS_SOFTMODEM_NOSTATS_BIT) && (NFAPI_MODE != NFAPI_MODE_VNF) && (NFAPI_MODE != NFAPI_MODE_AERIAL)) {
  //   //threadCreate(&proc->L1_stats_thread, nrL1_stats_thread, (void *)gNB, "L1_stats", -1, OAI_PRIORITY_RT_LOW);
  //   printf("L1 stats thread created.\n");
  // } else {
  //   printf("L1 stats thread not created due to conditions.\n");
  // }
...
void init_gNB()
{
  ...
  for (int inst = 0; inst < RC.nb_nr_L1_inst; inst++) {
    ...
    SM_Log_Assert(_GNB_APP_, (gNB->if_inst = NR_IF_Module_init(inst)) != NULL, "Cannot initialize the interface");
