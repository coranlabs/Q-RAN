...
static void *emulatedRF_thread(void *param) {
  ...
  while(!oai_exit) {
    ...
    ret = pthread_mutex_lock(&proc->mutex_emulateRF);
    if (ret != 0) {
        exit(EXIT_FAILURE);
    }
    ...
    ret = pthread_mutex_unlock(&proc->mutex_emulateRF);
    if (ret != 0) {
        exit(EXIT_FAILURE);
    }
...
void wakeup_slaves(RU_proc_t *proc) {
  ...
  for (int i=0; i<proc->num_slaves; i++) {
    ...
    ret = pthread_mutex_timedlock(&slave_proc->mutex_FH, &wait);
    if (ret != 0) {
        exit(EXIT_FAILURE);
    }
...
void wakeup_L1s(RU_t *ru) {
  ...
  if (proc->RU_mask[ru->proc.tti_rx] == (1<<eNB->num_RU)-1) { // all RUs have provided their information so continue on and wakeup eNB top
    ...
    if (pthread_mutex_unlock(&proc->mutex_RU) != 0) {
        fprintf(stderr, "Error while unlocking mutex_RU\n");
        exit(EXIT_FAILURE);
    }
...
void wakeup_prach_ru_br(RU_t *ru) {
  ...
  if (ret != 0) {
      fprintf(stderr, "[RU] ERROR pthread_mutex_lock for RU prach thread BR (IC %d)\n", ru->proc.instance_cnt_prach_br);
      exit(EXIT_FAILURE);
  }
  ...
  if (ru->proc.instance_cnt_prach_br==-1) {
    ...
    if (pthread_cond_signal(&ru->proc.cond_prach_br) != 0) {
        fprintf(stderr, "ERROR pthread_cond_signal for RU prach thread BR\n");
        exit(EXIT_FAILURE);
    }
...
static void *ru_thread_tx( void *param ) {
  ...
  while (!oai_exit) {
    ...
    for(int i = 0; i<ru->num_eNB; i++) {
      ...
      if (ret != 0) {
          fprintf(stderr, "Failed to unlock mutex, return code: %d\n", ret);
          exit(EXIT_FAILURE);
      }
        ...
        ret = pthread_mutex_unlock(&eNB_proc->mutex_RU_tx);
        if (ret != 0) {
            fprintf(stderr, "mutex_unlock failed with return code: %d\n", ret);
            exit(EXIT_FAILURE);
        }
        ...
        if (ret != 0) {
            fprintf(stderr, "mutex_lock failed with return code: %d\n", ret);
            exit(EXIT_FAILURE);
        }
...
static void *ru_thread( void *param ) {
  ...
  if(get_softmodem_params()->emulate_rf) {
    ...
    ret = pthread_mutex_lock(ru->ru_mutex);
    if (ret != 0) {
        fprintf(stderr, "mutex_lock failed with return code: %d\n", ret);
        exit(EXIT_FAILURE);
    }
  ...
  while (!oai_exit) {
    ...
    if(!(ru->emulate_rf)) {
      if (ru->is_slave == 0){
        ...
        // AssertFatal(ru->state == RU_SYNC || ru->state == RU_RUN ||
  ...
  //  LOG_I(PHY,"RU %d rf device ready\n",ru->idx);
    ...
    while (ru->state == RU_RUN || ru->state == RU_CHECK_SYNC) {
      ...
      if(first_phy_tx == 0) {
        ...
        ret = pthread_mutex_lock(&ru->proc.mutex_phy_tx);
        if (ret != 0) {
            fprintf(stderr, "[RU] Failed pthread_mutex_lock for phy tx thread (IC %d), return code: %d\n", ru->proc.instance_cnt_phy_tx, ret);
            exit(EXIT_FAILURE);
        }
        ...
        if (ru->proc.instance_cnt_phy_tx==-1) {
          ...
          if (pthread_cond_signal(&ru->proc.cond_phy_tx) != 0) {
              fprintf(stderr, "Failed pthread_cond_signal for phy_tx thread\n");
              exit(EXIT_FAILURE);
          }
      ...
      if (ru->wait_cnt > 0) {
        ...
        if (ret != 0) {
            fprintf(stderr, "[eNB] Failed to lock proc mutex for eNB pre scd, return code: %d\n", ret);
            exit(EXIT_FAILURE);
        }
        ...
        if (ret != 0) {
            fprintf(stderr, "[eNB] Failed to unlock mutex_pre_scd for eNB pre scd, return code: %d\n", ret);
            exit(EXIT_FAILURE);
        }
...
void init_ru_vnf(void) {
  ...
  // LOG_I(PHY,"configuring RU from file\n");
      SM_Logs(LOG_INFO, _ARIA_, "Radio Unit (RU) Configuration: Loading parameters from configuration file. Initializing RU operational profiles and resource bindings.\n");
