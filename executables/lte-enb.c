...
static inline int rxtx(PHY_VARS_eNB *eNB,
  ...
  if (NFAPI_MODE==NFAPI_MODE_VNF) {
    ...
    if (ret != 0) {
        fprintf(stderr, "[eNB] error unlocking proc mutex for eNB pre scd, return %d\n", ret);
    }
  }

#endif
  ret = pthread_mutex_lock(&eNB->UL_INFO_mutex);
  if (ret != 0) {
      fprintf(stderr, "error locking UL_INFO_mutex, return %d\n", ret);
      exit(EXIT_FAILURE); 
  }
  ...
  ret = pthread_mutex_unlock(&eNB->UL_INFO_mutex);
  if (ret != 0) {
      fprintf(stderr, "error unlocking UL_INFO_mutex, return %d\n", ret);
      exit(EXIT_FAILURE);
  }
...
static void *L1_thread_tx(void *param) {
  ...
  while (!oai_exit) {
    ...
    phy_procedures_eNB_TX(eNB, proc, 1);
    ret = pthread_mutex_lock(&proc->mutex);
    if (ret != 0) {
      fprintf(stderr, "error locking L1_proc_tx mutex, return %d\n", ret);
      exit(EXIT_FAILURE);
    }
    ...
    ret = pthread_cond_signal(&proc->cond);
    if (ret != 0) {
      fprintf(stderr, "error cond_signal mutex, return %d\n", ret);
      exit(EXIT_FAILURE);
    }

    ret = pthread_mutex_unlock(&proc->mutex);
    if (ret != 0) {
      fprintf(stderr, "error unlocking L1_proc_tx mutex, return %d\n", ret);
      exit(EXIT_FAILURE);
    }
...
int wakeup_txfh(PHY_VARS_eNB *eNB,
  ...
  if (release_thread(&proc->mutex_RUs, &proc->instance_cnt_RUs, "wakeup_txfh") != 0) {
      fprintf(stderr, "error releasing eNB lock on RUs\n");
      exit(EXIT_FAILURE);
  }
  ...
  if (waitret == ETIMEDOUT) {
    ...
    ret = pthread_mutex_lock(&eNB->proc.mutex_RU_tx);
    if (ret != 0) {
        fprintf(stderr, "mutex_lock returns %d\n", ret);
        exit(EXIT_FAILURE);
    }

    // AssertFatal((ret=pthread_mutex_lock(&eNB->proc.mutex_RU_tx))==0,"mutex_lock returns %d\n",ret);
    eNB->proc.RU_mask_tx = 0;
    ret = pthread_mutex_unlock(&eNB->proc.mutex_RU_tx);
    if (ret != 0) {
      fprintf(stderr, "mutex_unlock returns %d\n", ret);
      exit(EXIT_FAILURE);
    }

    // AssertFatal((ret=pthread_mutex_unlock(&eNB->proc.mutex_RU_tx))==0,"mutex_unlock returns %d\n",ret);

    ret = pthread_mutex_lock(&proc->mutex_RUs);
    if (ret != 0) {
        fprintf(stderr, "mutex_lock returns %d\n", ret);
        exit(EXIT_FAILURE);
    }
    ...
    ret = pthread_mutex_unlock(&proc->mutex_RUs);
    if (ret != 0) {
        fprintf(stderr, "mutex_unlock returns %d\n", ret);
        exit(EXIT_FAILURE);
    }
  ...
  for(int ru_id=0; ru_id<eNB->num_RU; ru_id++) {
    ...
    if (pthread_cond_signal(&ru_proc->cond_eNBs) != 0) {
        fprintf(stderr, "[eNB] ERROR pthread_cond_signal for eNB TXnp4 thread\n");
        exit(EXIT_FAILURE);
    }
    ...
    ret = pthread_mutex_unlock(&ru_proc->mutex_eNBs);
    if (ret != 0) {
        fprintf(stderr, "mutex_unlock returned %d\n", ret);
        exit(EXIT_FAILURE);
    }
...
int wakeup_tx(PHY_VARS_eNB *eNB,
  ...
  ret = pthread_mutex_lock(&L1_proc_tx->mutex);
  if (ret != 0) {
      fprintf(stderr, "mutex_lock returns %d\n", ret);
      exit(EXIT_FAILURE);
  }
  ...
  if (pthread_cond_signal(&L1_proc_tx->cond) != 0) {
      fprintf(stderr, "ERROR pthread_cond_signal for eNB L1 thread tx\n");
      exit(EXIT_FAILURE);
  }

  // AssertFatal(pthread_cond_signal(&L1_proc_tx->cond) == 0, "ERROR pthread_cond_signal for eNB L1 thread tx\n");

  ret = pthread_mutex_unlock(&L1_proc_tx->mutex);
  if (ret != 0) {
      fprintf(stderr, "mutex_unlock returns %d\n", ret);
      exit(EXIT_FAILURE);
  }
...
int wakeup_rxtx(PHY_VARS_eNB *eNB,
  ...
  ret = pthread_mutex_lock(&L1_proc->mutex);
  if (ret != 0) {
      fprintf(stderr, "mutex_lock returns %d\n", ret);
      exit(EXIT_FAILURE);
  }
  ...
  if (L1_proc->instance_cnt == 0) { // L1_thread is busy so abort the subframe
    ...
    ret = pthread_mutex_unlock(&L1_proc->mutex);
    if (ret != 0) {
        fprintf(stderr, "mutex_unlock return %d\n", ret);
        exit(EXIT_FAILURE);
    }
...
*/
  ...
  ret = pthread_cond_signal(&L1_proc->cond);
  if (ret != 0) {
      fprintf(stderr, "ERROR pthread_cond_signal for eNB RXn-TXnp4 thread, ret %d\n", ret);
      exit(EXIT_FAILURE);
  }
...
void wakeup_prach_eNB(PHY_VARS_eNB *eNB,
  ...
  if (ru!=NULL) {
    ...
    ret = pthread_mutex_lock(&proc->mutex_RU_PRACH);
    if (ret != 0) {
        fprintf(stderr, "mutex_lock return %d\n", ret);
        exit(EXIT_FAILURE);
    }
    ...
    if (proc->RU_mask_prach != (1<<eNB->num_RU)-1) {  // not all RUs have provided their information so return
      ret = pthread_mutex_unlock(&proc->mutex_RU_PRACH);
      if (ret != 0) {
          fprintf(stderr, "mutex_unlock return %d\n", ret);
          exit(EXIT_FAILURE);
      }
  ...
  if (is_prach_subframe(fp,frame,subframe)>0) {
    ...
    if (ret != 0) {
        fprintf(stderr, "[eNB] ERROR pthread_mutex_lock for eNB PRACH thread %d (IC %d)\n", proc->thread_index, proc->instance_cnt_prach);
        exit(EXIT_FAILURE);
    }
...
void wakeup_prach_eNB_br(PHY_VARS_eNB *eNB,
  ...
  if (ru!=NULL) {
    ...
    ret = pthread_mutex_lock(&proc->mutex_RU_PRACH_br);
    if (ret != 0) {
        fprintf(stderr, "mutex_lock return %d\n", ret);
        exit(EXIT_FAILURE);
    }
    ...
    if (proc->RU_mask_prach_br != (1<<eNB->num_RU)-1) {  // not all RUs have provided their information so return
      ...
      ret = pthread_mutex_unlock(&proc->mutex_RU_PRACH_br);
      if (ret != 0) {
          fprintf(stderr, "mutex_unlock return %d\n", ret);
          exit(EXIT_FAILURE);
      }
