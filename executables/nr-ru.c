...
//     SM_Logs(LOG_INFO, _ARIA_, "Radio UNIT (%d) has been allocated CPU:  %d",ru->idx, sched_getcpu());
...
//   SM_Logs(LOG_INFO, _PHY_, "RU Process Initialization: Instance %d | Fronthaul Protocol: %d | Synchronization Source: External Timing Device.\n",ru->idx, NB_functions[ru->function] );
...
//       SM_Logs(LOG_INFO, _PHY_, "Resource Allocation Summary: PHY Layer Instances: %d | RU Entities: %d | Allocated Compute Units (CU): %d.\n",RC.nb_L1_inst,RC.nb_RU,get_nprocs() );
...
//                     SM_Logs(LOG_INFO, _ARIA_, "GPIO Control Interface: Configured in 'GENERIC' mode. Awaiting specific operational assignments or control directives for Radio Unit hardware.\n");
...
//         SM_Logs(LOG_INFO, _PHY_, "Synchronizing temporal framework: Configuring clock reference to INTERNAL oscillator.\n");
//         RC.ru[j]->openair0_cfg.clock_source = internal;
//       }
...
//         SM_Logs(LOG_INFO, _PHY_, "Timebase calibration: Assigning INTERNAL source for temporal alignment.\n");
//         RC.ru[j]->openair0_cfg.time_source = internal;
//       }
...
int attach_rru(RU_t *ru) {
  ...
  while (received_capabilities==0) {
    ...
    if ((len = ru->ifdevice.trx_ctlrecv_func(&ru->ifdevice,
      ...
      // LOG_E(PHY,"Received incorrect message %d from RRU %d\n",rru_config_msg.type,ru->idx);
      SM_Logs(LOG_ERROR,_PHY_,"Incorrect message %d from RU %d\n",rru_config_msg.type,ru->idx);
    }
  }
...
void fh_if5_south_in(RU_t *ru,
  ...
  if (proc->first_rx == 0) {
    if (proc->tti_rx != *tti) {
      // LOG_E(PHY,"Received Timestamp doesn't correspond to the time we think it is (proc->tti_rx %d, subframe %d)\n",proc->tti_rx,*tti);
      SM_Logs(LOG_ERROR,_PHY_,"The received timestamp does not match the expected time RX : %d\n", proc->tti_rx);
...
void fh_if4p5_south_in(RU_t *ru,
  ...
  do {   // Blocking, we need a timeout on this !!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (packet_type == IF4p5_PULFFT) proc->symbol_mask[sl] = proc->symbol_mask[sl] | (1<<symbol_number);
      ...
      SM_Logs(LOG_ERROR,_PHY_,"Received ptick frame %d does not match the expected frame %d\n", f, *frame);

      } 
      // LOG_E(PHY,"rx_fh_if4p5: PULTICK received frame %d != expected %d\n",f,*frame);

      if ((proc->first_rx==0) && (sl!=*slot)){
        SM_Logs(LOG_ERROR,_PHY_,"Received ptick subframe %d does not match the expected subframe %d\n", sl, *slot,proc->first_rx);
  ...
  if (proc->first_rx == 0) {
    if (proc->tti_rx != *slot) {
      // LOG_E(PHY,"Received Timestamp (IF4p5) doesn't correspond to the time we think it is (proc->tti_rx %d, subframe %d)\n",proc->tti_rx,*slot);
      SM_Logs(LOG_ERROR,_PHY_,"Mismatch in received timestamp and expected timing rx: %d, subframe: %d\n", proc->tti_rx, *slot);
      exit_fun("Exiting");
    }

    if (proc->frame_rx != *frame) {
      // LOG_E(PHY,"Received Timestamp (IF4p5) doesn't correspond to the time we think it is (proc->frame_rx %d frame %d)\n",proc->frame_rx,*frame);
      SM_Logs(LOG_ERROR,_PHY_,"Mismatch in received timestamp and expected timing rx: %d, frame: %d\n", proc->frame_rx, *frame);
      exit_fun("Exiting");
    }
  } else {
    proc->first_rx = 0;
...
static void rx_rf(RU_t *ru, int *frame, int *slot)
{
  ...
  if (rxs != samples_per_slot) {
    //exit_fun( "problem receiving samples" );
    // LOG_E(PHY, "problem receiving samples\n");
    SM_Logs(LOG_ERROR,_PHY_,"While receiving samples");
  }
...
void *ru_thread(void *param)
{
  ...
  if (setup_RU_buffers(ru)!=0) {
    // LOG_E(PHY, "Exiting, cannot initialize RU Buffers\n");
    SM_Logs(LOG_ERROR,_PHY_,"Initializing RU buffer\n",ret);
    exit(-1);
  }
...
void init_NR_RU(configmodule_interface_t *cfg, char *rf_config_file)
{
  ...
  for (ru_id=0; ru_id<RC.nb_RU; ru_id++) {
    ...
    if (ru->if_south != REMOTE_IF4p5) {
      ...
      if (threadCnt < 2)
        // LOG_E(PHY, "Number of threads for gNB should be more than 1. Allocated only %d\n", threadCnt);
        SM_Logs(LOG_ERROR,_PHY_,"gnb thread allocation");
      char pool[80];
      int s_offset = sprintf(pool,"%d",ru->tpcores[0]);
      for (int icpu=1; icpu<threadCnt; icpu++) {
         s_offset+=sprintf(pool+s_offset,",%d",ru->tpcores[icpu]);
 ...
 // config_getlist(cfg, &RUParamList, RUParams, sizeofArray(RUParams), NULL);
  if (global_configs.numRUs > 0) {
    ...
    for (int j = 0; j < RC.nb_RU; j++) {
      ...
      RC.ru[j]->if_timing = synch_to_ext_device;
      if (RC.nb_nr_L1_inst > 0)
        RC.ru[j]->num_gNB =global_configs.numRUs;
      else
        RC.ru[j]->num_gNB  = 0;
      for (int i = 0; i < RC.ru[j]->num_gNB; i++)
        RC.ru[j]->gNB_list[i] = RC.gNB[0];
      ...
      //if (strcmp(*(RUParamList.paramarray[j][RU_LOCAL_RF_IDX].strptr), "yes") == 0) {
        ...
        //LOG_D(PHY, "RU %d: Transport %s\n", j, *(RUParamList.paramarray[j][RU_TRANSPORT_PREFERENCE_IDX].strptr));
        RC.ru[j]->eth_params.local_if_name = global_configs.RUs.LOCALifname;
        RC.ru[j]->eth_params.my_addr       = global_configs.RUs.LOCALaddr;
        RC.ru[j]->eth_params.remote_addr   = global_configs.RUs.REMOTEaddr;
        RC.ru[j]->eth_params.my_portc      = global_configs.RUs.LOCALportC;
        RC.ru[j]->eth_params.remote_portc  =  global_configs.RUs.REMOTEportC;
        RC.ru[j]->eth_params.my_portd      =global_configs.RUs.LOCALportD;
        RC.ru[j]->eth_params.remote_portd  = global_configs.RUs.REMOTEportD;
       ...
       // if (strcmp(*(RUParamList.paramarray[j][RU_TRANSPORT_PREFERENCE_IDX].strptr), "udp") == 0) {
        ...
        // } else if (strcmp(*(RUParamList.paramarray[j][RU_TRANSPORT_PREFERENCE_IDX].strptr), "udp_if4p5") == 0) {
          ...
          RC.ru[j]->function                     = NGFI_RAU_IF4p5;
          RC.ru[j]->eth_params.transp_preference = ETH_UDP_IF4p5_MODE;
        } else if (strcmp( global_configs.RUs.tr_preference, "raw_if4p5") == 0) {
          RC.ru[j]->if_south                     = REMOTE_IF4p5;
          RC.ru[j]->function                     = NGFI_RAU_IF4p5;
          RC.ru[j]->eth_params.transp_preference = ETH_RAW_IF4p5_MODE;
        }
      ...
      RC.ru[j]->nb_tx                             = global_configs.RUs.nb_tx;
      RC.ru[j]->nb_rx                             = global_configs.RUs.nb_rx;
      RC.ru[j]->att_tx                            = global_configs.RUs.att_tx;
      RC.ru[j]->att_rx                            =  global_configs.RUs.att_rx ;
      RC.ru[j]->if_frequency                      = global_configs.RUs.RU_IFfreq;
      RC.ru[j]->if_freq_offset                    = global_configs.RUs.RU_if_freqoffset;
      RC.ru[j]->sl_ahead                          = global_configs.RUs.sl_ahead;
      RC.ru[j]->num_bands                         = 1; //global_configs.RUs.bands;  will softcode it
      for (int i = 0; i < RC.ru[j]->num_bands; i++)
        RC.ru[j]->band[i] = global_configs.RUs.bands;
      RC.ru[j]->openair0_cfg.nr_flag              = global_configs.RUs.RU_nrflag;
      RC.ru[j]->openair0_cfg.nr_band              = RC.ru[j]->band[0];
      RC.ru[j]->openair0_cfg.nr_scs_for_raster    = global_configs.RUs.RU_nr_scsRaster;
      LOG_D(PHY,
            "[RU %d] Setting nr_flag %d, nr_band %d, nr_scs_for_raster %d\n",
            j,
            RC.ru[j]->openair0_cfg.nr_flag,
            RC.ru[j]->openair0_cfg.nr_band,
            RC.ru[j]->openair0_cfg.nr_scs_for_raster);
      RC.ru[j]->openair0_cfg.rxfh_cores[0]        = 0; //*(RUParamList.paramarray[j][RU_RXFH_CORE_ID].iptr);
      RC.ru[j]->openair0_cfg.txfh_cores[0]        = 0; //*(RUParamList.paramarray[j][RU_TXFH_CORE_ID].iptr);
      RC.ru[j]->num_tpcores                       = 2; //*(RUParamList.paramarray[j][RU_NUM_TP_CORES].iptr);
      RC.ru[j]->half_slot_parallelization         = 1; //*(RUParamList.paramarray[j][RU_HALF_SLOT_PARALLELIZATION].iptr);
      RC.ru[j]->ru_thread_core                    = global_configs.RUs.ru_thread_core;
                ;
      LOG_D(PHY, "[RU %d] Setting half-slot parallelization to %d\n", j, RC.ru[j]->half_slot_parallelization);
      //AssertFatal(RC.ru[j]->num_tpcores <= RUParamList.paramarray[j][RU_TP_CORES].numelt, "Number of TP cores should be <=16\n");
      for (int i = 0; i < RC.ru[j]->num_tpcores; i++)
        RC.ru[j]->tpcores[i] = -1; //RUParamList.paramarray[j][RU_TP_CORES].iptr[i];
    }// j=0..num_rus
  } else {
    RC.nb_RU = 0;
  } // setting != NULL
