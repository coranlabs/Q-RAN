...
void get_common_options(configmodule_interface_t *cfg, uint32_t execmask)
{
  ...
  nfapi_index = global_configs.defvals.NFAPI_idx;
  printf("nfapi_index: %d\n", nfapi_index);
  AssertFatal(nfapi_index >= 0,"Index for nfapi config option not found!");
  nfapi_mode = global_configs.defvals.NFAPI_mde;
  printf("nfapi_mode: %d\n", nfapi_mode);
  ...
  if(config_isparamset(cmdline_logparams,config_paramidx_fromname(cmdline_logparams,numparams, CONFIG_FLOG_OPT))) {
    printf("Entering condition: CONFIG_FLOG_OPT is set\n");
    set_glog_onlinelog(online_log_messages);
  }

  if(config_isparamset(cmdline_logparams,config_paramidx_fromname(cmdline_logparams,numparams, CONFIG_LOGL_OPT))) {
    printf("Entering condition: CONFIG_LOGL_OPT is set\n");
    set_glog(glog_level);
  }

  if (start_telnetsrv) {
    printf("Entering condition: Starting Telnet server\n");
    load_module_shlib("telnetsrv",NULL,0,NULL);
  }
  
  if (start_telnetclt) {
    printf("Entering condition: Starting Telnet client\n");
  ...
  if (noS1) {
    printf("Entering condition: noS1 is enabled\n");
    set_softmodem_optmask(SOFTMODEM_NOS1_BIT);
  }

  if (nonbiot) {
    printf("Entering condition: nonbiot is enabled\n");
    set_softmodem_optmask(SOFTMODEM_NONBIOT_BIT);
  }

  if (rfsim) {
    printf("Entering condition: rfsim is enabled\n");
    set_softmodem_optmask(SOFTMODEM_RFSIM_BIT);
  }

  if (do_forms) {
    printf("Entering condition: do_forms is enabled\n");
    set_softmodem_optmask(SOFTMODEM_DOSCOPE_BIT);
  }

  if (enable_imscope) {
    printf("Entering condition: enable_imscope is enabled\n");
    set_softmodem_optmask(SOFTMODEM_IMSCOPE_BIT);
  }

  if (start_websrv) {
    printf("Entering condition: Starting Web server\n");
