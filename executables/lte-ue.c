...
void init_thread(int sched_runtime,
  ...
  if (settingPriority) {
    if (CPU_COUNT(cpuset) > 0){
      SM_Log_Assert(_ARIA_,0 == pthread_setaffinity_np(pthread_self() , sizeof(cpu_set_t), cpuset), "" );
    }
      // AssertFatal( 0 == pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), cpuset), "");

    struct sched_param sp;
    sp.sched_priority = sched_fifo;
    SM_Log_Assert(_ARIA_,pthread_setschedparam(pthread_self(),SCHED_FIFO,&sp)==0 , "\n" );
...
void init_UE(int nb_inst,
  ...
  for (inst=0; inst<nb_inst; inst++) {
    ...
    SM_Log_Assert(_ARIA_,0 == pthread_create(&UE->proc.pthread_ue,
                                    &UE->proc.attr_ue,
                                    UE_thread,
                                    (void *)UE), "" );
...
static void *UE_thread_synch(void *arg) {
  ...
  */
  ...
  while (oai_exit==0) {
    SM_Log_Assert(_ARIA_, 0== pthread_mutex_lock(&UE->proc.mutex_synch), "");
    // AssertFatal ( 0== pthread_mutex_lock(&UE->proc.mutex_synch), "");

    while (UE->proc.instance_cnt_synch < 0)
      // the thread waits here most of the time
      pthread_cond_wait( &UE->proc.cond_synch, &UE->proc.mutex_synch );
    SM_Log_Assert(_ARIA_, 0== pthread_mutex_unlock(&UE->proc.mutex_synch), "");
...
static void *UE_phy_stub_single_thread_rxn_txnp4(void *arg) {
  ...
  thread_top_init("UE",1,870000L,1000000L,1000000L);
  // for multipule UE's L2-emulator
  //module_id_t Mod_id = 0;
  //int init_ra_UE = -1; // This counter is used to initiate the RA of each UE in different SFrames
  static __thread int UE_thread_rxtx_retval;
    ...
    if (is_synchronized == 0) {
      if (instance_cnt_synch < 0) {  // we can invoke the synch
        ...
        if (instance_cnt_synch == 0) {
          SM_Log_Assert(_ARIA_, 0 == pthread_cond_signal(&UE->proc.cond_synch), "");
      ...
      if (start_rx_stream==0) {
        ...
        SM_Log_Assert(_ARIA_,pthread_cond_signal(&proc->cond_rxtx) ==0,"");
        // AssertFatal (pthread_cond_signal(&proc->cond_rxtx) ==0,"");
        SM_Log_Assert(_ARIA_,pthread_mutex_unlock(&proc->mutex_rxtx) ==0,"");
...
static void *timer_thread( void *param ) {
  ...
  while (!oai_exit) {
    ...
    SM_Log_Assert(_ARIA_,pthread_mutex_unlock(&phy_stub_ticking->mutex_ticking) ==0,"");
