...
int attach_rru(RU_t *ru)
{
  ...
  while (received_capabilities==0) {
    ...
    if ((len = ru->ifdevice.trx_ctlrecv_func(&ru->ifdevice,
      ...
      SM_Logs(LOG_ERROR,_PHY_,"Incorrect message received from RU");
  ...
  while (!oai_exit) // Change the cond
      ...
      else
	{
	  switch(rru_config_msg.type)
        ...
        // SM_Logs(LOG_ERROR,_ARIA_,"RRU_capab message received\n");
    ...
    //  LOG_E(PHY,"RRU not ready, cannot start\n"); 
     SM_Logs(LOG_ERROR,_PHY_,"RRU is not prepared\n");
    ...
    //  LOG_E(PHY,"Received RRU_sync_ok from a master RRU...Ignoring\n"); 	
       ...
       SM_Logs(LOG_ERROR,_PHY_,"RRU is inactive\n");
		}
	      }else{}
