             ...
             uint8_t flag)
{
  ...
  devname = deflibname;
  //printf("devname in loadlib : %s\n",devname);

  ret=load_module_shlib(devname,shlib_fdesc,1,NULL);
  //printf("Hi i am after load_module_shlib: \n");
  AssertFatal( (ret >= 0),
  	           "Library %s couldn't be loaded\n",devname);
  //printf("Hi i am after load_module_shlib: 1\n");
  int return_value = ((devfunc_t)shlib_fdesc[0].fptr)(device, openair0_cfg, cfg);
  //printf("Return value from function pointer: %d\n", return_value);
                         ...
                         openair0_config_t *openair0_cfg)
{
  ...
  // printf("Hi i am after load_lib in openair0_device_load \n");
                            ...
                            eth_params_t *eth_params)
{
  ...
  // printf("Hi i am after load_lib in openair0_transport_load \n");
