...
//supporting functions for the table : 
double get_uptime_seconds() {
    time_t current_time = time(NULL);
    return difftime(current_time, app_start_time);
}
...
int xran_fh_rx_read_slot(ru_info_t *ru, int *frame, int *slot)
{
  ...
  *frame = oran_sync_info.f;
  ...
  // }
    ...
    if ((*frame & 0x7f) == 0 && *slot == 0 && xran_get_common_counters(gxran_handle, &x_counters[0]) == XRAN_STATUS_SUCCESS) {
      ...
      for (int o_xu_id = 0; o_xu_id < fh_init->xran_ports; o_xu_id++) {
        ...
        total_rx_packets += x_counters[o_xu_id].rx_counter;

        total_tx_packets += x_counters[o_xu_id].tx_counter;

        total_rx_bytes += x_counters[o_xu_id].rx_bytes_per_sec;

        total_tx_bytes += x_counters[o_xu_id].tx_bytes_per_sec;
        aria_ue_stats.counters_stats.stats_available = 1;
    ...
    //Print ORAN stats header



    // printf("\n\n-----------------------------------------------------\n");

    // LOG_I(NR_PHY, "ORAN STATS num_o_ru[1]:\n");
    ...
    double per_value = calculate_PER(x_counters[o_xu_id].Total_msgs_rcvd, x_counters[o_xu_id].Rx_corrupt);


    aria_ue_stats.counters_stats.per_value = per_value;
    aria_ue_stats.counters_stats.xran_ports = fh_init->xran_ports;
    aria_ue_stats.counters_stats.nb_rx =ru->nb_rx;
    aria_ue_stats.counters_stats.rx_bytes_counter = x_counters[o_xu_id].rx_bytes_counter;
    aria_ue_stats.counters_stats.rx_bytes_per_sec =  x_counters[o_xu_id].rx_bytes_per_sec;
    aria_ue_stats.counters_stats.Rx_corrupt =  x_counters[o_xu_id].Rx_corrupt;
    aria_ue_stats.counters_stats.rx_counter =  x_counters[o_xu_id].rx_counter;
    aria_ue_stats.counters_stats.Rx_early =  x_counters[o_xu_id].Rx_early;
    aria_ue_stats.counters_stats.Rx_late =  x_counters[o_xu_id].Rx_late;
    aria_ue_stats.counters_stats.Rx_on_time =  x_counters[o_xu_id].Rx_on_time;
    aria_ue_stats.counters_stats.Rx_pkt_dupl =  x_counters[o_xu_id].Rx_pkt_dupl;
    aria_ue_stats.counters_stats.rx_srs_packets =  x_counters[o_xu_id].rx_srs_packets;
    aria_ue_stats.counters_stats.Total_msgs_rcvd =  x_counters[o_xu_id].Total_msgs_rcvd;
    aria_ue_stats.counters_stats.tx_bytes_counter =  x_counters[o_xu_id].tx_bytes_counter;
    aria_ue_stats.counters_stats.tx_bytes_per_sec = x_counters[o_xu_id].tx_bytes_per_sec;
    aria_ue_stats.counters_stats.tx_counter = x_counters[o_xu_id].tx_counter;

    for (int rxant = 0; rxant < ru->nb_rx / fh_init->xran_ports; rxant++) { 
      aria_ue_stats.counters_stats.rx_prach_packets[rxant] = x_counters[o_xu_id].rx_prach_packets[rxant];
      aria_ue_stats.counters_stats.rx_pusch_packets[rxant] = x_counters[o_xu_id].rx_pusch_packets[rxant];
    }



    // SM_Logs(LOG_INFO, _PHY_,"\nRUNNING STATS:\n");
    // printf("|---------------------|--------------|\n");

    //     printf("| " GREEN "Rx_On_Time          " RESET "| %12ld |\n", x_counters[o_xu_id].Rx_on_time);
    //     printf("| "YELLOW "Rx_Early            " RESET "| %12ld |       ___________________       \n", x_counters[o_xu_id].Rx_early);
    // printf("| " LIGHT_RED "Rx_Late             " RESET "| %12ld |       |                   |      \n", x_counters[o_xu_id].Rx_late);
    //       printf("| " RED "Rx_Corrupt          " RESET "| %12ld |       |    " RED "%10s" RESET "     |      \n", x_counters[o_xu_id].Rx_corrupt,ARIA_NAME);
    //                       printf("| Rx_Pkt_Dupl         | %12ld |       |    Uptime :       |      \n", x_counters[o_xu_id].Rx_pkt_dupl);
    //                       printf("| Tx-bytesCount       | %12ld |       |  "YELLOW "   %02d:%02d:%02d" RESET "      |      \n", x_counters[o_xu_id].tx_bytes_counter, hours, minutes, seconds);
    //                       printf("| DL Throughput       | %12ld |kbps   |    [hh:mm:ss]     |      \n", x_counters[o_xu_id].tx_bytes_per_sec * 8 / 1000L);
    //                       printf("| Tx-Packets_PerSec   | %12ld |       |                   |      \n", x_counters[o_xu_id].tx_counter);
    //                       printf("| Rx-bytesCount       | %12ld |       |___________________|      \n", x_counters[o_xu_id].rx_bytes_counter);
    //                       printf("| UL Throughput       | %12ld |kbps                             \n", x_counters[o_xu_id].rx_bytes_per_sec * 8 / 1000L);
    //                       printf("| Rx-Packets_PerSec   | %12ld |                                 \n", x_counters[o_xu_id].rx_counter);
    //                       printf("| PER                 | %12ld |                                 \n", per_value);
    //                       printf("|---------------------|--------------|\n");
    //                       printf("| Total_Rx-Packets    | %12ld |\n", x_counters[o_xu_id].Total_msgs_rcvd);
    //                       printf("| Num-Rx_OnTime       | %12ld |\n", x_counters[o_xu_id].Total_msgs_rcvd-x_counters[o_xu_id].Rx_early-x_counters[o_xu_id].Rx_late);
    //                       printf("|---------------------|--------------|\n");

    // //Print PUSCH and PRACH counts for each antenna
    // printf(YELLOW "\nPUSCH / PRACH [PACKETS]\n" RESET);
    // printf("|--------------------------------------------------------------------------\n");


    // printf("|--------------------------------------------------------------------------\n");
    // printf("| Num-SRS_Packets     | %12ld |                                    |\n", x_counters[o_xu_id].rx_srs_packets);
    ...
    // {
    //   printf("|" YELLOW " UE ID (RNTI)      " RESET "  | %12ld | %12s |\n", aria_ue_stats.stats[i].ue_rnti, aria_ue_stats.stats[i].sync_status);
    //   // printf("| %12s |\n", aria_ue_stats.stats[i].sync_status);
    //   printf("|---------------------|--------------|                                    |\n");
    //   printf("| CU UE ID            | %12ld |" YELLOW " Signal Quality :" RESET "                   |\n", aria_ue_stats.stats[i].cu_ue_id);
    //   printf("| num_meas            | %12d | CQI                 | %12ld |\n", aria_ue_stats.stats[i].num_meas, aria_ue_stats.stats[i].cqi);
    //   printf("| pcmax               | %12ld | Average RSRP        | %12d |\n", aria_ue_stats.stats[i].pcmax,aria_ue_stats.stats[i].avg_rsrp);
    //   printf("| PMI                 | %10ld.%ld | Pathloss            | %12d |\n", aria_ue_stats.stats[i].pmi[0],aria_ue_stats.stats[i].pmi[1],aria_ue_stats.stats[i].ph);
    //   printf("|--------------------------------------------------------------------------\n");
    //   printf("|" YELLOW " DL" RESET "                                 | " YELLOW "UL" RESET "                                 |\n");
    //   printf("|--------------------------------------------------------------------------\n");
    //   printf("| RBs                 | %12ld | RBs                 | %12ld |\n", aria_ue_stats.stats[i].dl_current_rbs, aria_ue_stats.stats[i].ul_current_rbs);
    //   printf("| RBs Retx            | %12ld | RBs Retx            | %12ld |\n", aria_ue_stats.stats[i].dl_total_rbs_retx, aria_ue_stats.stats[i].ul_total_rbs_retx);
    //   printf("| Total RBs           | %12ld | Total RBs           | %12ld |\n", aria_ue_stats.stats[i].dl_total_rbs, aria_ue_stats.stats[i].ul_total_rbs);
    //   printf("| Bytes               | %12ld | Bytes               | %12ld |\n", aria_ue_stats.stats[i].dl_current_bytes, aria_ue_stats.stats[i].ul_current_bytes);
    //   printf("| HARQ Errors         | %12ld | HARQ Errors         | %12ld |\n", aria_ue_stats.stats[i].dlsch_errors, aria_ue_stats.stats[i].ulsch_errors);
    //   printf("| MCS                 | %12ld | MCS                 | %12ld |\n", aria_ue_stats.stats[i].mcs_dl, aria_ue_stats.stats[i].mcs_ul);
    //   printf("| BLER                | %12.5f | BLER                | %12.5f |\n", aria_ue_stats.stats[i].bler_dlsch, aria_ue_stats.stats[i].bler_ulsch);
    //   printf("|--------------------------------------------------------------------------\n");
    //   printf("| Total Bytes         | %12ld | Total Bytes         | %12ld |\n", aria_ue_stats.stats[i].mac_tx_bytes, aria_ue_stats.stats[i].mac_rx_bytes);
    //   printf("|--------------------------------------------------------------------------\n");


    //   // printf("|             | %12ld |\n", aria_ue_stats.stats[i].num_meas);

    //   // printf("| Num of UEs          | %12ld |\n", aria_ue_stats.num_of_ue);
  ...
  //Print overall ORAN stats

      //  LOG_I(NR_PHY, " |----------------------|-------------|\n");

      //  LOG_I(NR_PHY, " | Total Tx Packets   | %12ld |\n", total_tx_packets);
      //  SM_Logs(LOG_INFO, _PHY_," | ------------------ |--------------|\n");

      //  LOG_I(NR_PHY, " | Total Tx Throughput| %12ld |\n\n", total_rx_bytes);
      //  SM_Logs(LOG_INFO, _PHY_," | ------------------ |--------------|\n");
  }
  return (0);
}
