...
{
  DevAssert(fhip != NULL);
  int num_dev = global_configs.fhi_72.num_dpdk_devs;
  ...
  for (int i = 0; i < num_dev; ++i)
    io_cfg->dpdk_dev[i] = strdup(global_configs.fhi_72.dpdk_devices[i]);
  //io_cfg->bbdev_dev = NULL;
  io_cfg->bbdev_mode = XRAN_BBDEV_NOT_USED; // none
  io_cfg->dpdkIoVaMode = 0; /* IOVA mode */
  io_cfg->dpdkMemorySize = 0; //*gpd(fhip, nump, ORAN_CONFIG_DPDK_MEM_SIZE)->uptr;
  io_cfg->core = global_configs.fhi_72.io_core;
  io_cfg->system_core = global_configs.fhi_72.system_core;
  io_cfg->pkt_proc_core = global_configs.fhi_72.pkt_proc_core;
  io_cfg->pkt_proc_core_64_127 = 0x0; // bitmap 0 -> no core
  io_cfg->pkt_aux_core = 0; /* sapmle app says 0 = "do not start" */
  io_cfg->timing_core = global_configs.fhi_72.timingcores; /* sample app: equal to io_core */
  //io_cfg->port = {0}; // all 0
  io_cfg->io_sleep = 0; // no sleep
  io_cfg->nEthLinePerPort = global_configs.fhi_72.nEthLinePerPort ;
  io_cfg->nEthLineSpeed = global_configs.fhi_72.nEthLineSpeed;
...
bool set_fh_init(struct xran_fh_init *fh_init)
{
  ...
  int num_rus = global_configs.numfhi_72;
  int num_ru_addr = global_configs.fhi_72.num_ruAddr;
  int num_du_addr =  global_configs.fhi_72.num_duAddr;
  int num_vfs = global_configs.fhi_72.num_dpdk_devs;
  ...
  if (!set_fh_io_cfg(&fh_init->io_cfg, fhip, nump))
    return false;
  //printf("Hi i am outta here: \n");
  if (!set_fh_eaxcid_conf(&fh_init->eAxCId_conf, XRAN_CATEGORY_A))
    return false;
  //printf("Outta here too: \n");
  fh_init->xran_ports = num_rus;
  fh_init->dpdkBasebandFecMode = 0;
  fh_init->dpdkBasebandDevice = NULL;
  fh_init->filePrefix = strdup(global_configs.fhi_72.fileprefix); // see DPDK --file-prefix
  fh_init->mtu =global_configs.fhi_72.mtu;
  //printf("Outta here too: 1\n");

  // if multiple RUs: xran_ethdi_init_dpdk_io() iterates over
  // &p_o_ru_addr[i]
  char **du_addrs = global_configs.fhi_72.du_addr;
  //printf("Outta here too: 2\n");
                                ...
                                struct xran_prach_config *prach_config)
{
  ...
  prach_config->eAxC_offset = global_configs.fhi_72.prach_config.eAxC_offset;

  g_kbar = global_configs.fhi_72.prach_config.kbar;
 // printf("Hi i'm outta here: set_fh_prach_config \n");
...
static bool set_fh_ru_config(const paramdef_t *rup, uint16_t fftSize, int nru, struct xran_ru_config *ru_config)
{
  ...
  ru_config->iqWidth = global_configs.fhi_72.ru_config.iq_width;
  ...
  ru_config->iqWidth_PRACH = global_configs.fhi_72.ru_config.iq_width_prach;
...
bool set_fh_config(int ru_idx, int num_rus, const openair0_config_t *oai0, struct xran_fh_config *fh_config)
{
  ...
  const int nfh = sizeofArray(FHconfigs);
  //config_getlist(config_get_if(), &FH_ConfigList, FHconfigs, nfh, aprefix);
  if (global_configs.fhi_72.num_fh_config == 0) {
  ...
  fh_config->Tadv_cp_dl = global_configs.fhi_72.fh_config.Tadv_cp_dl;
  ...
  //   return false;
  fh_config->T2a_min_cp_dl = global_configs.fhi_72.fh_config.T2a_cp_dl.min;
  fh_config->T2a_max_cp_dl = global_configs.fhi_72.fh_config.T2a_cp_dl.max;

  fh_config->T2a_min_cp_ul = global_configs.fhi_72.fh_config.T2a_cp_ul.min;
  fh_config->T2a_max_cp_ul = global_configs.fhi_72.fh_config.T2a_cp_ul.max;

  fh_config->T2a_min_up = global_configs.fhi_72.fh_config.T2a_up.min;  
  fh_config->T2a_max_up = global_configs.fhi_72.fh_config.T2a_up.max;

  fh_config->Ta3_min = global_configs.fhi_72.fh_config.Ta3.min;
  fh_config->Ta3_max = global_configs.fhi_72.fh_config.Ta3.max;

  fh_config->T1a_min_cp_dl = global_configs.fhi_72.fh_config.T1a_cp_dl.min;
  fh_config->T1a_max_cp_dl = global_configs.fhi_72.fh_config.T1a_cp_dl.max;

  fh_config->T1a_min_cp_ul = global_configs.fhi_72.fh_config.T1a_cp_ul.min;
  fh_config->T1a_max_cp_ul = global_configs.fhi_72.fh_config.T1a_cp_ul.max;

   fh_config->T1a_min_up = global_configs.fhi_72.fh_config.T1a_up.min;
  fh_config->T1a_max_up = global_configs.fhi_72.fh_config.T1a_up.max;

  fh_config->Ta4_min = global_configs.fhi_72.fh_config.Ta4.min;
  fh_config->Ta4_max = global_configs.fhi_72.fh_config.Ta4.max;
  ...
  fh_config->cp_vlan_tag = global_configs.fhi_72.cp_vlan_tag;
  fh_config->up_vlan_tag = global_configs.fhi_72.cp_vlan_tag;
  fh_config->debugStop = 0;
  fh_config->debugStopCount = 0;
  fh_config->DynamicSectionEna = 0;
  fh_config->GPS_Alpha = 0;
  fh_config->GPS_Beta = 0;
  //printf("I'm boutta go into set_fh_prach_config: \n ");
